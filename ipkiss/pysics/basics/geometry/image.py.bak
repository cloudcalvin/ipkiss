...
class __ImageGeometry2D__(__Geometry2D__):
    ...
    def size_info(self):
        return SizeInfo(
            west=self.center[0] - 0.5 * self.size[0],
...
class __ImageMaterialGeometry2D__(__ImageGeometry2D__):
    ...
    def get_material(self, coordinate):
        pixel_coord = (int(self.coordinate[0] / self.grid.x + 0.5),
                       int(self.coordinate[1] / self.grid.y + 0.5))
...
class __GrayscaleImageMaterialGeometry2D__(__ImageMaterialGeometry2D__):
    ...
    def color_to_material(self, color):
        ...
        return BlendedMaterial(
            material_1=self.material_white,
            material_2=self.material_black,
            fraction=fraction)
...
class __LayerSuperpositionMaterialGeometry2D__(__ImageGeometry2D__):
    ...
    layer_to_material_map = DictProperty(
        required=True)  #key: set with layers / value : material
    layer_superposition_array = NumpyArrayProperty(
        required=True
    )  #numpy array with in every point of the grid the list of layers at that point
    grid = FloatProperty(required=True)

    def __init__(self, **kwargs):
        super(__LayerSuperpositionMaterialGeometry2D__,
    ...
    def get_material(self, coordinate):
        ...
        pixel_coord = (int(coordinate[0] / self.grid), int(
            coordinate[1] / self.grid))
        try:
...
class __ProcessSuperpositionMaterialGeometry2D__(__ImageGeometry2D__):
    ...
    process_to_material_map = DictProperty(
        required=True)  #key: set with processes / value : material
    process_flags = DictProperty(
        required=True
    )  #key : process / value : numpy array with True/False flag at every point
    grid = FloatProperty(required=True)
    ...
    def __processes_at_coordinate(self, coord):
        ...
        for process in self.processes:
            process_active_at_coord = self.process_flags[process][coord[0],
                                                                  coord[1]]
