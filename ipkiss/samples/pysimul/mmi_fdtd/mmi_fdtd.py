...
simul_params["sources"] = [
    ModeProfileContinuousSourceAtPort(
        center_wavelength=center_wavelength,
        smoothing_width=60.0,
        port=mmi.west_ports[0].transform_copy(transformation=Translation(
            translation=(-1.0, 0.0))),
        polarization=TE,
        stop_time=120.0)
]
...
simul_params["datacollectors"] = [
    FluxplaneAtPort(
        ...
        name="flux input"), FluxplaneAtPort(
            center_wavelength=center_wavelength,
            pulse_width=fluxplane_width,
            number_of_sampling_freq=100,
            port=mmi.east_ports[0],
            overlap_trench=False,
            name="flux upper output"), FluxplaneAtPort(
                center_wavelength=center_wavelength,
                pulse_width=fluxplane_width,
                number_of_sampling_freq=100,
    ...
    ProbingpointAtPort(port=mmi.east_ports[0].transform_copy(
        transformation=Translation(translation=(0.25, 0.0))))
]
...
simul_params["step_processor"] = SaveFieldsHDF5Processor(
    H5OutputIntervalSteps=100, field_component=compHz)

#specify a stop criterium : stop when the Hz field component at the probing point has decayed to 0.01 of it's peak value
simul_params["stopcriterium"] = RunUntilFieldsDecayedAtProbingPoint(
    field_component=compHz,
    probingpoint=simul_params["datacollectors"][3],
    decay_factor=0.01)
...
known_efficiency = [
    0.95
]  #we know from earlier simualtions that the efficiency of the MMI is 95%
