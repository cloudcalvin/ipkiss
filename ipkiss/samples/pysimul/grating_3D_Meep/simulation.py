...
grating_unit_cell = WgGratingPeriodShallow(
    length=period, wg_definition=wg_def_cavity, shallow_process=TECH.PROCESS.FCW
)
reference_component = GratingCavityWithModeFilter(
...
act_port_offset = 2.5
p_e = reference_component.east_ports[0].transform_copy(
    Translation(translation=(-section_len, 0))
)
p_w = reference_component.west_ports[0].transform_copy(
    Translation(translation=(+section_len, 0))
)
...
reference_component_simulator = StructureMeep3DSimulator(
    ...
    growth=0.0,
    wavelength=1.55,
    source_input_port_offset=-section_len / 2.0,
)  # offset for source in reference to the input port
...
reference_component_simulator.add_output_cut(
    OutputCut(normal_vector="y", filename="%s/Ey_y_ref" % output_directory)
)  # slice orthogonal to the Y-axis at y=0
reference_component_simulator.add_output_cut(
    OutputCut(
        normal_vector="z", cut_value=0.810, filename="%s/Ey_z1_ref" % output_directory
    )
)  # slice orthogonal to the Z-axis at z=0.81 (700nm SiOx + half of 220nm Si = 700+110 = 810 => 0.81)
...
reference_component_simulator.stop_time_multiplier = (
    2.0
)  # stop after 2 times the pulse length (pulse length calculated by Meep)
reference_component_simulator.dft_terms = (
    500
)  # number of sampling points for DFT flux plane
...
grating_unit_cell = WgGratingPeriodShallow(
    length=period, wg_definition=wg_def_cavity, shallow_process=TECH.PROCESS.FC
)
component = GratingCavityWithModeFilter(
    wg_definition=wg_def_cavity,
    period=period,
    cavity_length=period / 2.0,
    number_of_periods_left=np_l,
...
reference_component_simulator.add_output_cut(
    OutputCut(normal_vector="y", filename="%s/Ey_y" % output_directory)
)
reference_component_simulator.add_output_cut(
    OutputCut(normal_vector="z", cut_value=0.81, filename="%s/Ey_z1" % output_directory)
)

reference_component_simulator.save_reversed_flux_to_file_ID = (
    -1
)  # don't save a flux, but initiliaze the flux from file_ID = 0 (see below)
reference_component_simulator.save_reversed_flux_to_file_filename = filename
reference_component_simulator.load_reversed_flux_from_file_filename = filename
reference_component_simulator.load_reversed_flux_from_file_ID = (
    0
)  # initiliaze the flux from file_ID = 0
