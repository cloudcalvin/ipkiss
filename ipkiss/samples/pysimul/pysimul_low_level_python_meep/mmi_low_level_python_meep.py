...
simul_params[
    ...
    10.0
)  # we want to put a point source far left of the west port, therefore, we extend the component at the ports with a waveguide of 10 micron
...
class MyProcedure(LowLevelPythonMeepProcedure):
    def run(self, interactive_mode=False):
        ...
        pulse_width_freq = (
            (float(pulse_width) / 1000.0) / (float(center_wavelength) / 1000.0)
        ) * center_freq
        ...
        source_position_vec = self.make_meep_vec(
            mmi.west_ports[0]
            .transform_copy(Translation(translation=(-9.0, 0)))
            .position
        )
