...
class ElementsExample(Structure):
    def define_elements(self, elems):
        ...
        for i in range(5, 50, 5):
            # horizontal line
            L = Line(
                layer=Layer(0), begin_coord=(0, i), end_coord=(2 * i, i), line_width=1.0
            )
            lines += L
            # diagonal line
        ...
        for i in range(5, 30, 5):
            ...
            rectangles += RoundedRectangle(
                layer=Layer(0), center=(xpos, -100), box_size=(2 * i, 50), radius=1
            )
        ...
        for i in range(5, 10):
            ...
            polygons += RegularPolygonPath(
                layer=Layer(0), center=(xpos, 0), radius=30, n_o_sides=i, line_width=2.0
            )
            xpos += 80
        ...
        # 1 hexagon structure
        hexagon = Structure(
            "hexagon", Hexagon(layer=Layer(0), center=(0.0, 0.0), radius=hexagon_radius)
        )
        ...
        lattice_vector = (
            lattice_constant,
            sqrt(3) * lattice_constant,
        )  # translation in X and Y
        lattice += ARef(
            unit_cell, origin=(0.0, 0.0), period=lattice_vector, n_o_periods=n_o_periods
        )
        ...
        tic_tac_toe += Line(Layer(0), (50, 0), (50, -150), 3) + Line(
            Layer(0), (100, 0), (100, -150), 3
        )  # vertical lines
        tic_tac_toe += Line(Layer(0), (0, -50), (150, -50), 3) + Line(
            Layer(0), (0, -100), (150, -100), 3
        )  # vertical lines
