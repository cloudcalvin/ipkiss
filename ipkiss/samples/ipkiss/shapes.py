...
class ShapesExample(Structure):
    def define_elements(self, elems):
        ...
        s = Shape(
            [(10.0, 0.0), (15.0, 10.0), (0.0, 10.0), (0.0, 5.0), (-15.0, 5.0),
             (-5.0, 0.0), (-10.0, -10.0), (-5.0, -15.0), (10.0, -15.0),
             (5.0, -10.0), (5.0, -5.0)],
            closed=True)
        ...
        t = ShapeGrow(s, 1.0)
        S9 = Structure("shape_grow",
                       Boundary(Layer(1), t) + Boundary(Layer(0), s))

        #round the shape with a given radius
        t = ShapeRound(original_shape=s, radius=2.0)
        S10 = Structure("shape_round",
                        Boundary(Layer(1), t) + Boundary(Layer(0), s))

        elems += [
            SRef(S1, (0.0, 200.0)), SRef(S2, (50.0, 200.0)),
            SRef(S3, (100.0, 200.0)), SRef(S4, (0.0, 150.0)),
            SRef(S5, (50.0, 150.0)), SRef(S6, (100.0, 150.0)),
            SRef(S7, (0.0, 100.0)), SRef(S8, (50.0, 100.0)),
            SRef(S9, (100.0, 100.0)), SRef(S10, (150.0, 100.0))
        ]
