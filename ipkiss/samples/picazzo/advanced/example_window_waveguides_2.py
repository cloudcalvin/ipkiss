...
class LshapeWgDefinition(WindowWaveguideDefinition):
    ...
    polarity = StringProperty(
        required=True,
        restriction=RestrictValueList(allowed_values=["left", "right"]))

    def define_windows(self):
        windows = []
        windows.append(
            PathWindow(
                layer=PPLayer(TECH.PROCESS.FC, TECH.PURPOSE.LF.LINE),
        ...
        if (self.trench_width > 0.0):
            windows.append(
                PathWindow(
                    layer=PPLayer(TECH.PROCESS.FC, TECH.PURPOSE.LF_AREA),
...
class LshapeWgToWgElDefinitionTaper(__WgElPortTaper__):
    ...
    def define_elements(self, elems):
        ...
        taper_elem1 = Boundary(
            layer=wg_lfarea_window.layer,
            shape=Shape([(wg_lfarea_window.start_offset,
                          0), (sign(wg_lfarea_window.end_offset) *
                               (abs(wg_lfarea_window.end_offset)), 0),
                         (wg_lfarea_window.end_offset, self.length),
                         (wg_lfarea_window.start_offset, self.length)]))
        taper_elements += taper_elem1
        taper_elem2 = Boundary(
            layer=wg_line_window.layer,
            shape=Shape([(wg_line_window.start_offset,
                          0), (wg_line_window.end_offset,
                               0), (wg_line_window.end_offset, self.length),
                         (wg_line_window.start_offset, self.length)]))

        taper_elements += taper_elem2
        taper_elem3 = Boundary(
            layer=fc_line_window.layer,
            shape=Shape([(fc_line_window.start_offset,
                          0), (fc_line_window.end_offset,
                               0), (fc_line_window.end_offset, self.length),
                         (fc_line_window.start_offset, self.length)]))
        taper_elements += taper_elem3

        taper_elem4 = Boundary(
            layer=fc_lfarea_window.layer,
            shape=Shape([(fc_lfarea_window.start_offset,
                          0), (fc_lfarea_window.end_offset,
                               0), (fc_lfarea_window.end_offset, self.length),
                         (fc_lfarea_window.start_offset, self.length)]))
        taper_elements += taper_elem4

        taper_elem5 = Boundary(
            layer=wg_lfarea_window.layer,
            shape=Shape([(wg_lfarea_window.start_offset, self.length),
                         (fc_lfarea_window.end_offset,
                          self.length), (sign(fc_lfarea_window.end_offset) *
                                         (abs(fc_lfarea_window.end_offset)),
                                         0), (fc_lfarea_window.end_offset, 0),
        ...
        taper_elements_transformed = taper_elements.transform_copy(
            transformation=Translation(translation=self.start_port.position) +
            Rotation(
                rotation=(self.start_port.angle - 90.0),
                rotation_center=self.start_port.position))
        return taper_elements_transformed

    def define_ports(self, ports):
        from ipkiss.plugins.photonics.port import *
        ports += OutOpticalPort(
            position=(self.start_port.position.x +
                      self.length * cos(self.start_port.angle_rad),
                      self.start_port.position.y +
                      self.length * sin(self.start_port.angle_rad)),
    ...
    def validate_properties(self):
        if not isinstance(self.start_port.wg_definition, LshapeWgDefinition):
            raise Exception(
                "Start port should have a waveguide definition of type 'LshapeWgDefinition'"
            )
        return True
...
if __name__ == "__main__":
    ...
    class Example(Structure):
        def define_elements(self, elems):
            my_path_shape = Shape([(0.0, 0.0), (50.0, 25.0), (100.0, 30.0),
                                   (150.0, 10.0)])
            ...
            taper = LshapeWgToWgElDefinitionTaper(
                start_port=wg_el.ports[1], length=20.0)
            taper1 = LshapeWgToWgElDefinitionTaper(
                start_port=wg_el.ports[0], length=20.0)

            struct = TaperDeepPorts(structure=Structure(
                elements=[wg_el, taper, taper1],
                ports=taper.ports + taper1.ports))
