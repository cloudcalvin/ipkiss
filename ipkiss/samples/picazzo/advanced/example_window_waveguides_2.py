...
class LshapeWgDefinition(WindowWaveguideDefinition):
    ...
    polarity = StringProperty(
        required=True, restriction=RestrictValueList(allowed_values=["left", "right"])
    )
...
class LshapeWgToWgElDefinitionTaper(__WgElPortTaper__):
    ...
    def define_elements(self, elems):
        ...
        taper_elem1 = Boundary(
            ...
            shape=Shape(
                [
                    (wg_lfarea_window.start_offset, 0),
                    (
                        sign(wg_lfarea_window.end_offset)
                        * (abs(wg_lfarea_window.end_offset)),
                        0,
                    ),
        ...
        taper_elem4 = Boundary(
            ...
            shape=Shape(
                [
                    (fc_lfarea_window.start_offset, 0),
        ...
        taper_elem5 = Boundary(
            ...
            shape=Shape(
                [
                    ...
                    (fc_lfarea_window.end_offset, self.length),
                    (
                        sign(fc_lfarea_window.end_offset)
...
if __name__ == "__main__":
    ...
    class Example(Structure):
        def define_elements(self, elems):
            my_path_shape = Shape(
                [(0.0, 0.0), (50.0, 25.0), (100.0, 30.0), (150.0, 10.0)]
            )
            ...
            struct = TaperDeepPorts(
                structure=Structure(
                    elements=[wg_el, taper, taper1], ports=taper.ports + taper1.ports
                )
            )
