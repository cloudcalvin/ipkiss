    ...
    def __init__(self, simul_volume, meep_volume):
        try:
            ...
            from pysics.materials.electromagnetics import (
                transform_material_stack_matrix_in_chi3_matrix
            )
            ...
            self.set_matrix_2D(
                self.material_chi3_matrix, meep_volume, resolution_factor
            )
            LOG.debug(
...
class MeepMaterial2DMatrix(Meep.CallbackMatrix2D):
    def __init__(self, simul_volume, meep_volume):
        ...
        self.material_eps_matrix = transform_material_stack_matrix_in_effective_index_epsilon_matrix(
            material_matrix, simul_volume.geometry.material_stack_factory
        )
        self.set_matrix_2D(self.material_eps_matrix, meep_volume, resolution_factor)
        LOG.debug(
            "Meep node %i -MeepMaterial2D (epsilon) object initialized."
            % int(Meep.my_rank())
        )
...
class MeepMaterial2DPolygons(__MeepMaterialPolygons__, Meep.PolygonCallback2D):
    def __init__(self, simul_volume, meep_volume):
        ...
        if not isinstance(simul_volume, __SimulationVolume__):
            raise AttributeException(
                "Parameter simul_volume of class MeepMaterial2DPolygons must be of type __SimulationVolume__"
            )
        ...
        for material_stack_id, bitmap_polygon in bitmap_polygons[
            1:
        ]:  # ignore canvas polygon
            eps = simul_volume.geometry.material_stack_factory[
                material_stack_id
...
class MeepMaterial3DPolygons(__MeepMaterialPolygons__, Meep.PolygonCallback3D):
    def __init__(self, simul_volume, meep_volume):
        ...
        self.add_material_stacks_from_numpy_matrix(
            simul_volume.geometry.material_stack_factory.get_numpy_matrix_representation_of_all_material_stacks(),
...
class AmplitudeFactor(Meep.Callback):
    ...
    def complex_vec(self, vec):
        ...
        try:
            ...
            LOG.debug(
                "Meep node %i -Amplitude factor for x=%f - y=%f is: %f \n"
...
class MeepSimulationEngine(FDTDEngine):
    ...
    def initialise_engine(self, landscape):
        ...
        [self.meepVol, self.dim] = self.__createMeepComputationalVolume(
        ...
        if self.dim == 2:
            ...
            self.material = MeepMaterial3DPolygons(
                landscape.simulation_volume, self.meepVol
            )
        ...
        self.structure = Meep.structure(
            self.meepVol, Meep.EPS, pml, symmetry=symmetry_object
        )
    ...
    def __createMeepComputationalVolume(self, volume):
        ...
        if isinstance(volume, SimulationVolume2D):
            return [
                Meep.vol2d(volume.window_width, volume.window_height, self.resolution),
                2,
            ]
    ...
    def __addMeepSource(self, src, meep_fields):
        ...
        if isinstance(src, __GaussianSource__):
            pw = (
                (float(src.pulse_width) / 1000.0)
                / (float(src.center_wavelength) / 1000.0)
            ) * center_freq
            meepSource = Meep.gaussian_src_time(center_freq, pw)
        if isinstance(src, __ContinuousSource__):
            meepSource = Meep.continuous_src_time(
                center_freq,
                src.smoothing_width,
                src.start_time,
                src.stop_time,
        ...
        if isinstance(src, __EMPointSource__):
            ...
            LOG.debug(
                "Meep node %i -Creating volume for source plane..." % (self.node_nr)
            )
            meepSrcVol = Meep.volume(vec1, vec2)
            print(
                "Meep node %i - source plane between points (%f , %f) and (%f , %f)."
                % (self.node_nr, vec1.x(), vec1.y(), vec2.x(), vec2.y())
            )
            LOG.debug(
                "Meep node %i -Now adding the volume source to Meep..." % (self.node_nr)
            )
            if isinstance(src, __AmplitudeShapedSource__):
                ...
                meep_fields.add_volume_source(
                    meepComp, meepSource, meepSrcVol, Meep.AMPL
                )
            else:
                meep_fields.add_volume_source(
                    meepComp, meepSource, meepSrcVol, src.amplitude
                )
        else:
    ...
    def __addMeepFluxplane(self, flx, meep_fields):
        ...
        print(
            "Meep node %i : flux plane between points (%f , %f) and (%f , %f) "
            % (self.node_nr, vec1.x(), vec1.y(), vec2.x(), vec2.y())
        )
        meepFlxVol = Meep.volume(vec1, vec2)
        center_freq = 1.0 / (float(flx.center_wavelength) / 1000.0)
        pw = (
            (float(flx.pulse_width) / 1000.0) / (float(flx.center_wavelength) / 1000.0)
        ) * center_freq
        ...
        setattr(
            flx, "scale", lambda factor: self.__scaleFluxplane(meepFluxplane, factor)
        )
    ...
    def __saveFluxToHDF5(self, pFluxplane, filename):
        LOG.debug(
            "Meep node %i -Saving flux to HDF5 in file %s ..."
    ...
    def exportDielectricH5(self, fileName=None):
        ...
        LOG.debug(
            "Meep node %i -Exporting dielectric to H5-file : %s"
            % (self.node_nr, fileName)
        )
        dielectric_file_handle = Meep.prepareHDF5File(fileName)
        self.meep_fields.output_hdf5(
            Meep.Dielectric, self.meepVol.surroundings(), dielectric_file_handle
        )
    ...
    def save_dielectricum_image(self, filename=None):
        ...
        if Meep.am_master():
            LOG.debug(
                "Meep node %i -Dielectricum saved to file %s..."
                % (self.node_nr, filename)
            )
            if self.dim == 2:
                cmd = (
                    "h5topng -a /home/emmanuel/workspace/Meep_tutorial/colormaps/yarg "
                    + filename
                )
                os.system(cmd)
        return filename
    ...
    def get_material_dataset(self):
        ...
        d = numpy.array(
            [
                self.landscape.simulation_volume.window_width,
                self.landscape.simulation_volume.window_height,
            ]
        )
        ...
        if (abs(ds.shape[0] - epsDim[0]) > 1) or (abs(ds.shape[1] - epsDim[1]) > 1):
            raise PythonSimulateException(
                "MeepSimulationEngine::visualizeDielectric - invalid dimension of HDF5 data. Expected %s and got %s"
                % (str(tuple(epsDim)), str(ds.shape))
            )
    ...
    def getFieldAmplitudeAtMonitorPoint(self, pCoord, pComp):
        ...
        if self.meep_fields == None:
            raise PythonSimulateException(
                "MeepSimulationEngine::getFieldAtMonitorPoint - call initialise_engine first."
            )
    ...
    def prepareHDF5File(self, filename):
        ...
        LOG.debug(
            "Meep node %i -Preparing HDF5 file '%s'..." % (self.node_nr, filename)
        )
        f = open(filename, "w")
        f.close()
    ...
    def writeFieldsToHDF5File(self, pFileRef, pComp, surroundings=None):
        ...
        if pFileRef == None:
            raise PythonSimulateException(
                "MeepSimulationEngine::getFieldAtMonitorPoint - call prepareHDF5File first."
            )
        c = self.__makeMeepComponent(pComp)
    ...
    def closeHDF5File(self, pFileRef):
        ...
        if Meep.am_master():
            ...
            if pFileRef == None:
                raise PythonSimulateException(
                    "MeepSimulationEngine::getFieldAtMonitorPoint - call prepareHDF5File first : cannot close a file that is not open."
                )
            del (pFileRef)
