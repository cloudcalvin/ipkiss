...
class MeepMaterial2DMatrix(Meep.CallbackMatrix2D):
    def __init__(self, simul_volume, meep_volume):
        ...
        self.set_matrix_2D(self.material_eps_matrix, meep_volume,
                           resolution_factor)
        LOG.debug("Meep node %i -MeepMaterial2D (epsilon) object initialized."
                  % int(Meep.my_rank()))
...
class MeepMaterial2DPolygons(__MeepMaterialPolygons__, Meep.PolygonCallback2D):
    def __init__(self, simul_volume, meep_volume):
        ...
        for material_stack_id, bitmap_polygon in bitmap_polygons[
                1:]:  #ignore canvas polygon
            eps = simul_volume.geometry.material_stack_factory[
                material_stack_id].effective_index_epsilon
...
class MeepMaterial3DPolygons(__MeepMaterialPolygons__, Meep.PolygonCallback3D):
    def __init__(self, simul_volume, meep_volume):
        ...
        self.add_material_stacks_from_numpy_matrix(
            simul_volume.geometry.material_stack_factory.
            get_numpy_matrix_representation_of_all_material_stacks(),
            simul_volume.geometry.material_stack_factory.
            get_number_of_material_stacks_in_store())
        for material_stack_id, bitmap_polygon in bitmap_polygons[
                1:]:  #ignore canvas polygon
...
class MeepSimulationEngine(FDTDEngine):
    ...
    def __createMeepComputationalVolume(self, volume):
        ...
        if isinstance(volume, SimulationVolume3D):
            return [
                Meep.vol3d(volume.window_width, volume.window_height,
                           volume.size_z, self.resolution), 3
            ]
        if isinstance(volume, SimulationVolume2D):
            return [
                Meep.vol2d(volume.window_width, volume.window_height,
                           self.resolution), 2
            ]
    ...
    def __addMeepSource(self, src, meep_fields):
        ...
        if isinstance(src, __ContinuousSource__):
            meepSource = Meep.continuous_src_time(
                center_freq, src.smoothing_width, src.start_time,
    ...
    def exportDielectricH5(self, fileName=None):
        ...
        LOG.debug("Meep node %i -Exporting dielectric to H5-file : %s" %
                  (self.node_nr, fileName))
        dielectric_file_handle = Meep.prepareHDF5File(fileName)
        self.meep_fields.output_hdf5(Meep.Dielectric,
                                     self.meepVol.surroundings(),
                                     dielectric_file_handle)
    ...
    def save_dielectricum_image(self, filename=None):
        ...
        if (Meep.am_master()):
            LOG.debug("Meep node %i -Dielectricum saved to file %s..." %
                      (self.node_nr, filename))
            if self.dim == 2:
    ...
    def get_material_dataset(self):
        ...
        for x in range(0, x_range):
            for y in range(0, y_range):
                mat[x, y] = Material(
                    name="dielectricum_material", epsilon=ds[x, y])
        return mat
    ...
    def prepareHDF5File(self, filename):
        ...
        LOG.debug("Meep node %i -Preparing HDF5 file '%s'..." % (self.node_nr,
                                                                 filename))
        f = open(filename, 'w')
        f.close()
