...
class __AmplitudeShapedSource__(__EMSource__):
    def get_amplitude_factor(self, coordinate):
        raise PythonSimulateException(
            "__AmplitudeShapedSource__ class should implement : get_amplitude_factor"
        )
...
class Fluxplane(__DataCollector__):
    ...
    def save_hdf5(self, filename):
        raise NotImplementedException(
            "No default implementation. Can be set with an engine-specific implementation upon initialisation."
        )
