...
def create_animated_gif_from_hdf5(
    field_filename, eps_filename, remove_png=True, uniform_colormap_range=True
):
    f = h5py.File(field_filename, "r")
    ds = list(f.items())[0][1]
    max_time = ds.shape[2] - 1
    print(
        "Creating animated gif from files %s and %s (time steps : %i)."
