...
class __CamfrGeometryCoordinateConverter__(StrongPropertyInitializer):
    ...
    def __call__(self, geometry_coordinate):
        ...
        if (
            (cx < 0)
            or (cx > self.camfr_box_size_x)
            or (cz < 0)
            or (cz > self.camfr_box_size_z)
        ):
...
class CamfrEngine(ModeSolverEngine):
    ...
    save_images = BoolProperty(
        default=True,
        doc="Export images (and csv files) of the mode profile and the geometry during the simulation.",
    )
    ...
    def set_camfr_settings(self, camfr_technology):
        ...
        if (self.camfr_technology.WAVELENGTH > 1.7) or (
            self.camfr_technology.WAVELENGTH < 1.0
        ):
            raise Exception(
    ...
    def get_camfr_stack_expr_for_geometry(
        ...
        for slab_spec in slabs_specifications[1 : max_slabs + 1]:
            ...
            LOG.debug(
                "Camfr slab %i : z-width = %f - materials: "
            ...
            LOG.debug(
                "Camfr: cumulative width of slabs : %f" % (stack_width * grid_step)
            )
    ...
    def field_for_geometry(
        ...
        while (not validation) and (iterations_counter < maximum_faulty_iterations):
            ...
            for z_position in camfr_z_positions:
                ...
                geometry_y_positions = numpy.arange(
                    si.south, si.north, field_extraction_grid
                )
                ...
                if self.save_images:
                    ...
                    numpy.savetxt(
                        csv_file_handle, numpy.array(data_for_csv), delimiter=", "
                    )
            ...
            if not validation:
                self.camfr_technology.WAVELENGTH = (
                    self.camfr_technology.WAVELENGTH + delta_wavelength_faulty_result
                )
                LOG.warning(
