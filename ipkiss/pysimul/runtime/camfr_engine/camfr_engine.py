...
def inc_field_func(x):
    inc_field_x_coord_new = inc_field_parameters.get_parameter(
        "inc_field_x_coord")

    inc_field_field_value_real_array = inc_field_parameters.get_parameter(
        "inc_field_field_value_real")
    inc_field_field_value_imag_array = inc_field_parameters.get_parameter(
        "inc_field_field_value_imag")
    ...
    if x_modi > -1.03 or x_modi < -1.47:  # co ordinate of the waveguide
        ...
        f1 = numpy.interp(x_modi, inc_field_x_coord,
                          inc_field_field_value_real)
        f2 = numpy.interp(x_modi, inc_field_x_coord,
                          inc_field_field_value_imag)
        f = f1 + f2 * (1.0j)
    return f
...
class __CamfrGeometryCoordinateConverter__(StrongPropertyInitializer):
    ...
    def __call__(self, geometry_coordinate):
        cx, cz = self.camfr_x(geometry_coordinate), self.camfr_z(
            geometry_coordinate)
        if (cx < 0) or (cx > self.camfr_box_size_x) or (cz < 0) or (
                cz > self.camfr_box_size_z):
            raise PythonSimulateException(
                "Coordinate out of bounds of geometry box")
...
class CamfrEngine(ModeSolverEngine):
    camfr_technology = RestrictedProperty(
        default=TECH.CAMFR, restriction=RestrictType(TechnologyTree))
    save_images = BoolProperty(
        default=True,
        doc=
        "Export images (and csv files) of the mode profile and the geometry during the simulation."
    )
    ...
    def initialise_engine_for_landscape(self, landscape):
        ...
        if (n_o_sources != 1):
            raise PythonSimulateException(
                "Expected exactly 1 source and got %i...." % n_o_sources)
        elif type(landscape.sources[0]) != ContinuousPointSource:
            raise PythonSimulateException(
                "Expected a datasource of type ContinuousPointSource, but got type %s..."
                % str(type(landscape.sources[0])))
        self.camfr_technology = copy.deepcopy(TECH.CAMFR)
        self.camfr_technology.WAVELENGTH = landscape.sources[
            0].center_wavelength
    ...
    def set_camfr_settings(self, camfr_technology):
        ...
        LOG.debug(
            "Camfr wavelength : %f micron" % self.camfr_technology.WAVELENGTH)
        ...
        LOG.debug("Camfr polarization : %s" % str(
            self.camfr_technology.POLARIZATION))
    ...
    def set_mode_profile_field(self, camfr_stack, inc_field, si_south):
        ...
        inc_field_parameters.set_parameter("inc_field_field_value_real",
                                           inc_field[1])
        inc_field_parameters.set_parameter("inc_field_field_value_imag",
                                           inc_field[2])
        inc_field_parameters.set_parameter("si_south", si_south)

        inc_field_eps = 1
        return camfr_stack.set_inc_field_function(inc_field_func,
                                                  inc_field_eps)
    ...
    def get_camfr_stack_expr_for_geometry(
        ...
        for eps_column, delta_column in zip(eps, deltas):
            if (any(delta_column)):
                ...
                slabs_specifications.append((current_slab_eps,
                                             current_slab_pixel_width))
                #start a new slab
        ...
        for slab_spec in slabs_specifications[1:max_slabs + 1]:
            ...
            for eps, delta in zip(eps_column, deltas):
                if (delta != 0):
                    material_specifications.append((current_material_eps,
                                                    current_material_height))
    ...
    def field_for_geometry(
        ...
        while (not validation) and (iterations_counter <
            ...
            for z_position in camfr_z_positions:
                ...
                fp = ElectroMagneticFieldProfile2D(
                    positions=[
