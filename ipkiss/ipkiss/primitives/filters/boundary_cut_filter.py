...
class BoundaryCutFilter(Filter):
    ...
    def __filter_Boundary__(self, item, DEBUG_ID=None):
        ...
        if len(item.shape) <= self.max_vertex_count:
            ...
            if self.save_debug_gds:  #DEBUG
                ...
                OP2 = FileOutputGdsii(
                    FileName="debug_struct_boundary_cut_%s_ORIGINAL.gds" %
                    (DEBUG_ID),
            ...
            cutting_y = si.south + (
                si.north - si.south
            ) / 3.0  #do not take 2, to avoid particular problems with Y-symmetric structures !
            ...
            while (len(transition_points) > 0) and (
                (abs(transition_points[0].index - transition_points[1].index)
                 == 1) or
                (transition_points[0].index == 0
                 and transition_points[1].index == len(points) - 1) or
            ...
            if len(transition_points) == 0:
                ...
                if len(
                        item.shape.intersections(
                            other_shape=Shape(points=[tp1, tp2]))) > 2:
                    result_boundaries = self.__rotate_and_filter__(
                        item, DEBUG_ID=DEBUG_ID)
                else:
                    ...
                    boundary1 = Boundary(
                        layer=item.layer,
                        shape=Shape(points_piece1, closed=True))
                    boundary2 = Boundary(
                        layer=item.layer,
                        shape=Shape(points_piece2, closed=True))
                ...
                if self.save_debug_gds:  #DEBUG
                    ...
                    my_lib += debug_struct
                    OP = FileOutputGdsii(
                        FileName="debug_struct_boundary_cut_%s.gds" %
                        (DEBUG_ID),
                        cut_boundaries=False)
                    OP.write(my_lib)
    ...
    def __rotate_and_filter__(self, item, DEBUG_ID):
        ...
        rot_item = item.transform_copy(
            transformation=Rotation(rotation=rot_angle)).flat_copy()
        rot_boundaries = self.__filter_Boundary__(
            rot_item, DEBUG_ID=DEBUG_ID + "_ROT")
        result = [
            b.transform_copy(
                transformation=Rotation(rotation=-rot_angle)).flat_copy()
