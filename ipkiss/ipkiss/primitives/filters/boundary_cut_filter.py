...
class BoundaryCutFilter(Filter):
    ...
    def __filter_Boundary__(self, item, DEBUG_ID=None):
        ...
        if len(item.shape) <= self.max_vertex_count:
            ...
            if self.save_debug_gds:  # DEBUG
                ...
                OP2 = FileOutputGdsii(
                    FileName="debug_struct_boundary_cut_%s_ORIGINAL.gds" % (DEBUG_ID),
                    cut_boundaries=False,
                )
                OP2.write(lib_debug)
                print(
                    DEBUG_ID,
                    " -- DECISION CRITERIUM len(item.shape) = ",
                    len(item.shape),
                )
            ...
            si = item.size_info()
            cutting_y = (
                si.south + (si.north - si.south) / 3.0
            )  # do not take 2, to avoid particular problems with Y-symmetric structures !
            ...
            transition_points = sorted(
                transition_points_unsorted,
                cmp=lambda p1, p2: int(numpy.sign(p1.x - p2.x)),
            )

            # if the 2 first transition points are consecutive points in the list of points, then ignore these
            while (len(transition_points) > 0) and (
                (abs(transition_points[0].index - transition_points[1].index) == 1)
                or (
                    transition_points[0].index == 0
                    and transition_points[1].index == len(points) - 1
                )
                or (
                    transition_points[1].index == 0
                    and transition_points[0].index == len(points) - 1
                )
            ):
            ...
            if len(transition_points) == 0:
                ...
                if self.save_debug_gds:  # DEBUG
                    ...
                    DEBUG_trans_point_path = Path(
                        layer=TECH.PPLAYER.FC.TEXT,
                        shape=Shape(transition_points),
                        line_width=0,
                    )
                ...
                if (
                    len(item.shape.intersections(other_shape=Shape(points=[tp1, tp2])))
                    > 2
                ):
                    result_boundaries = self.__rotate_and_filter__(
                        item, DEBUG_ID=DEBUG_ID
                    )
                else:
                    points_piece1 = points[tp1.index : tp2.index + 1]
                    points_piece2 = points[tp2.index :] + points[: tp1.index + 1]
                    boundary1 = Boundary(
                        layer=item.layer, shape=Shape(points_piece1, closed=True)
                    )
                    boundary2 = Boundary(
                        layer=item.layer, shape=Shape(points_piece2, closed=True)
                    )
                    # make a recursive call to filter the two pieces in turn
                    filtered_boundaries1 = self.__filter_Boundary__(
                        boundary1, DEBUG_ID=DEBUG_ID + "_1"
                    )
                    filtered_boundaries2 = self.__filter_Boundary__(
                        boundary2, DEBUG_ID=DEBUG_ID + "_2"
                    )
                ...
                if self.save_debug_gds:  # DEBUG
                    ...
                    debug_struct = Structure(
                        elements=result_boundaries + DEBUG_trans_point_path
                    )
                    my_lib = Library(name="DEBUG")
                    my_lib += debug_struct
                    OP = FileOutputGdsii(
                        FileName="debug_struct_boundary_cut_%s.gds" % (DEBUG_ID),
                        cut_boundaries=False,
                    )
                    OP.write(my_lib)
    ...
    def __rotate_and_filter__(self, item, DEBUG_ID):
        ...
        rot_item = item.transform_copy(
            transformation=Rotation(rotation=rot_angle)
        ).flat_copy()
        rot_boundaries = self.__filter_Boundary__(rot_item, DEBUG_ID=DEBUG_ID + "_ROT")
        result = [
            b.transform_copy(transformation=Rotation(rotation=-rot_angle)).flat_copy()
            for b in rot_boundaries
        ]
        return result
