...
class StructureVisualizationAspect(object):
    def __initialize(self, resolution=resolution_default, **kwargs):
        ...
        if "vfabrication_process_flow" in kwargs:
            params["vfabrication_process_flow"] = kwargs[
                "vfabrication_process_flow"]
    ...
    def visualize_2d(self,
                     ...
                     save_to_image=False,
                     save_to_image_path="./",
                     save_to_image_filename_without_ext=None,
                     fig_size_factor=0.1,
                     enter_tk_mainloop=True,
        ...
        vis = SimulationVolumeVisualization2D(
            simulation_volume=self.simul_def.landscape.simulation_volume)
        fig = vis.visualize(aspect_ratio_equal)
        if show:
            from ipkiss.visualisation.show import show
            show(
                fig,
                title=self.simul_def.landscape.simulation_volume.name,
                enter_tk_mainloop=enter_tk_mainloop)
        if save_to_image:
            ...
            if not size is None:
                ...
                fig.set_size_inches((si.width * fig_size_factor,
                                     si.height * fig_size_factor))
            ...
            LOG.debug(
                "Virtual fabrication imaged saved as %s.\n>>> Use INKSCAPE to visualize and zoom in on the file (free download at inkscape.org)"
                % filename_svg)
            return filename_png

    def visualize_3d_povray(self,
                            camera_pos,
                            look_at=(0, 0, 1),
                            z_extrusion_factor=1.0):
        self.__initialize()
        from pysimul.visualization.visualization import SimulationVolumeVisualization3D
        vis = SimulationVolumeVisualization3D(
            simulation_volume=self.simul_def.landscape.simulation_volume)
        vis.make_povray_file(
            camera_pos=camera_pos,
            look_at=look_at,
            z_extrusion_factor=z_extrusion_factor)
    ...
    def __generate_3d_hdf5__(self,
        ...
        if not hasattr(
                self,
                "__eps_hdf5_generated__") or not self.__eps_hdf5_generated__:
            simul_params = dict()
            simul_params["structure"] = self
            simul_params["resolution"] = resolution
            meep_engine = __meep_engine_class__(
            ...
            if meep_engine_type == 0:
                simul_def.procedure.run(
                    filename=filename_without_ext + ".eps.h5")
            else:
    ...
    def visualize_3d_x_crosssection(self, x_co, resolution=resolution_default):
        ...
        meep_engine = self.__generate_3d_hdf5__(filename_without_ext,
                                                resolution)
        ...
        target_filename = filename_without_ext + "_section_x_" + str(
            x_co) + ".png"
        cmd_make_section_png = "h5topng -x " + str(
            x_co_meep
        ) + " " + filename_without_ext + ".eps.h5 -o " + target_filename
        import os
        if (os.name != "posix"):
            raise Exception(
                "The HDF5 file was created, but could not automatically trigger the h5topng command to generate a cross-section : linux installation needed for that.\n%s"
                % str(cmd_make_section_png))
        result = self.__run_os_cmd__(cmd=cmd_make_section_png)
        if result != 0:
            raise Exception("The following OS command returned an error : %s" %
                            cmd_make_section_png)
        result = self.__run_os_cmd__(cmd="eog " + target_filename)
        if result != 0:
            raise Exception("The following OS command return and error : %s" %
                            ("eog " + target_filename))
    ...
    def visualize_3d_y_crosssection(self, y_co, resolution=resolution_default):
        ...
        target_filename = filename_without_ext + "_section_y_" + str(
            y_co) + ".png"
        cmd_make_section_png = "h5topng -y " + str(
            y_co_meep
        ) + " " + filename_without_ext + ".eps.h5 -o " + target_filename
        import os
        if (os.name != "posix"):
    ...
    def visualize_3d_vtk(self, resolution=resolution_default):
        ...
        if (os.name != "posix"):
            raise Exception(
                "The HDF5 file was created, but could not automatically trigger the h5tovtk command to generate a VTK file : linux installation needed for that.\n%s"
                % str(cmd_make_vtk))
...
class __StructuresVFabricationDialog__(tkinter.simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        self.label = Label(
            master,
            text=
            "Select a structure and click OK to trigger the virtual fabrication."
        )
        self.label.pack()
        #checkbox
        ...
        self.checkbox = Checkbutton(
            master,
            text="Aspect ration equal",
            variable=self.aspect_ratio_equal_int)
        ...
        self.listbox = Listbox(
            master,
            height=min(20, len(self.library.structures)),
            width=max_len)
        yscroll = Scrollbar(
            master, command=self.listbox.yview, orient=VERTICAL)
...
class LibraryVisualizationAspect(object):
    def select_structures(self, func=lambda x: x):
        ...
        while 1:
            ...
            d = __StructuresVFabricationDialog__(root, self,
                                                 aspect_ratio_equal)
            if d.result:
    ...
    def __do_visualize_2d_for_struct__(self, struct, **kwargs):
        struct.visualize_2d(
            save_to_image=False, enter_tk_mainloop=False, **kwargs)

    def visualize_structures_2d(self):
        self.select_structures(
            LibraryVisualizationAspect.__do_visualize_2d_for_struct__)
    ...
    def visualize_structures_3d(self, resolution=60):
        ...
        self.select_structures(
            LibraryVisualizationAspect.__do_visualize_3d_for_struct__)
