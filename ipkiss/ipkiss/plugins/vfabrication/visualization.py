...
class StructureVisualizationAspect(object):
        ...
        **kwargs
        ...
        if save_to_image:
            ...
            if not size is None:
                ...
                fig.set_size_inches(
                    (si.width * fig_size_factor, si.height * fig_size_factor)
                )
    ...
    def visualize_3d_povray(
        self, camera_pos, look_at=(0, 0, 1), z_extrusion_factor=1.0
    ):
        self.__initialize()
    ...
    def __generate_3d_hdf5__(self, filename_without_ext, resolution=resolution_default):
        ...
        if (
            not hasattr(self, "__eps_hdf5_generated__")
            or not self.__eps_hdf5_generated__
        ):
            simul_params = dict()
    ...
    def visualize_3d_x_crosssection(self, x_co, resolution=resolution_default):
        ...
        if result != 0:
            raise Exception(
                "The following OS command returned an error : %s" % cmd_make_section_png
            )
        result = self.__run_os_cmd__(cmd="eog " + target_filename)
        if result != 0:
            raise Exception(
                "The following OS command return and error : %s"
                % ("eog " + target_filename)
            )
    ...
    def visualize_3d_y_crosssection(self, y_co, resolution=resolution_default):
        ...
        if result != 0:
            raise Exception(
                "The following OS command returned an error : %s"
                % ("eog " + target_filename)
            )
    ...
    def visualize_3d_vtk(self, resolution=resolution_default):
        ...
        if result != 0:
            raise Exception(
                "The following OS command returned an error : %s" % cmd_make_vtk
            )
...
class __StructuresVFabricationDialog__(tkinter.simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        self.label = Label(
            master,
            text="Select a structure and click OK to trigger the virtual fabrication.",
        )
        self.label.pack()
        # checkbox
        self.aspect_ratio_equal_int = IntVar()
        self.checkbox = Checkbutton(
            master, text="Aspect ration equal", variable=self.aspect_ratio_equal_int
        )
        ...
        self.listbox = Listbox(
            master, height=min(20, len(self.library.structures)), width=max_len
        )
