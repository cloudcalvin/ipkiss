...
__all__ = [
    "OpticalPort", "InOpticalPort", "OutOpticalPort", "OpticalPortProperty",
    "VerticalOpticalPort"
]
...
class OpticalPort(__OpticalPort__, __OrientedPort__):
    ...
    def transform(self, transformation):
        if transformation.is_isometric():
            ...
            self.position = transformation.apply_to_coord(self.position)
            self.angle_deg = transformation.apply_to_angle_deg(self.angle_deg)
            self.wg_definition = self.wg_definition.transform_copy(
                transformation)
        return self
    ...
    def transform_copy(self, transformation):
        ...
        return self.__class__(
            position=transformation.apply_to_coord(self.position),
    ...
    def is_match(self, other):
        return ((self.position == other.position)
                and (self.angle - other.angle) % 360.0 == 180.0
                and (self.wg_definition == other.wg_definition))
    ...
    def __ne__(self, other):
        return (self.position != other.position
                ...
                or (self.wg_definition != other.wg_definition))

    def __repr__(self):
        return "<OpticalPort: (%f, %f), a=%f, D=%s>" % (self.position[0],
                                                        self.position[1],
                                                        self.angle_deg,
                                                        self.wg_definition)
    ...
    def get_corner1(self):
        ...
        port_corner1_x = port_position[0] + (
            wg_width / 2.0) * cos(port_angle - pi / 2.0)
        port_corner1_y = port_position[1] + (
            wg_width / 2.0) * sin(port_angle - pi / 2.0)
...
class VerticalOpticalPort(__OpticalPort__, __OutOfPlanePort__):
    ...
    def __repr__(self):
        return "<VerticalOpticalPort: (%f, %f), a=%f, i=%f>" % (
            self.position[0], self.position[1], self.angle_deg,
