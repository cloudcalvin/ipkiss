...
from ipkiss.aspects.port.port import (
    __InPort__,
    __OutPort__,
    __OrientedPort__,
    __OutOfPlanePort__,
...
class OpticalPort(__OpticalPort__, __OrientedPort__):
    ...
    def __eq__(self, other):
        if not isinstance(other, OpticalPort):
            return False
        return (
            self.position == other.position
    ...
    def __repr__(self):
        return "<OpticalPort: (%f, %f), a=%f, D=%s>" % (
            self.position[0],
            self.position[1],
            self.angle_deg,
            self.wg_definition,
    ...
    def get_corner1(self):
        ...
        port_corner1_x = port_position[0] + (wg_width / 2.0) * cos(
            port_angle - pi / 2.0
        )
        port_corner1_y = port_position[1] + (wg_width / 2.0) * sin(
            port_angle - pi / 2.0
        )
    ...
    def get_corner2(self):
        ...
        port_angle = self.angle * DEG2RAD
        wg_width = self.wg_definition.wg_width
        port_corner2_x = port_position[0] + (wg_width / 2.0) * cos(
            port_angle + pi / 2.0
        )
        port_corner2_y = port_position[1] + (wg_width / 2.0) * sin(
            port_angle + pi / 2.0
        )
