...
class WgElBundleWaveguides(Group):
    ...
    waveguides = RestrictedProperty(
        restriction=RestrictTypeList(Group), required=True
    )  # maybe waveguide later on
    ...
    def define_elements(self, elems):
        ...
        for (wg1, Wg2) in zip(self.waveguides[:-1], self.waveguides[1:]):
            ...
            if D1 > D_ref:
                ST_2 += S1[-1].move_polar_copy(
                    0.5 * w1 + t1, A1[-2] + orientation * 90.0
                )
                ST_2 += S2[-1].move_polar_copy(
                    0.5 * w2 + t2, A2[-2] - orientation * 90.0
                )
...
class WgElBundleGeneric(WgElBundleWaveguides):
    ...
    def define_waveguides(self):
        if len(self.shapes) != len(self.wg_widths) or len(self.wg_widths) != len(
            self.trench_widths
        ):
        ...
        waveguides = [
            ...
            for (S, w, t) in zip(self.shapes, self.wg_widths, self.trench_widths)
        ]
        return waveguides
...
class WgElBundleConnectRoundedGeneric(__RoundedWaveguide__, WgElBundleGeneric):
    ...
    def define_waveguides(self):
        waveguides = []
        for (S, w, t, r) in zip(
            self.shapes, self.wg_widths, self.trench_widths, self.bend_radii
        ):
...
class WgElBundleConnectRounded(
    __IdenticalWaveguides__, WgElBundleConnectRoundedGeneric
):
