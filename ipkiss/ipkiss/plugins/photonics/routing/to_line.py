...
__all__ = [
    "RouteToAngle", "RouteToSouth", "RouteToSouthAtMaxX", "RouteToSouthAtMinX",
    "RouteToSouthAtX", "RouteToHorizontal", "RouteToHorizontalAtY",
    "RouteToWest", "RouteToWestAtMaxY", "RouteToWestAtMinY", "RouteToWestAtY",
    "RouteToLine", "RouteToManhattan", "RouteToParallelLine", "RouteToEast",
    "RouteToEastAtMaxY", "RouteToEastAtMinY", "RouteToEastAtY", "RouteToNorth",
    "RouteToNorthAtMaxX", "RouteToNorthAtMinX", "RouteToNorthAtX",
    "RouteToVertical", "RouteToVerticalAtX",
...
class RouteToAngle(__RouteBasic__):
    ...
    def define_points(self, pts):
        ...
        if angle == 0.0:
            pts = points_add_polar(pts,
                                   max(self.start_straight, self.end_straight),
                                   self.angle_out)
        elif angle == 180.0:
            bs1_90, bs2_90 = self.get_bend90_size()
            pts = points_add_polar(pts, self.start_straight + bs1_90,
                                   self.input_port.angle_deg)
            pts = points_add_polar(pts, self.min_straight + bs1_90 + bs2_90,
                                   self.input_port.angle_deg + 90.0)
            pts = points_add_polar(pts, self.end_straight + bs2_90,
...
class RouteToParallelLine(__RouteToLine__):
    ...
    def define_points(self, pts):
        ...
        line2 = straight_line_from_point_angle(self.input_port.position,
                                               self.input_port.angle_deg)
        ...
        if line == line2:
            ...
            angle = self.find_angle(D, self.max_s_bend_angle, 0.0,
                                    self.s_bend_angle_resolution)
            ...
            P = self.input_port.position.move_polar_copy(
                self.start_straight + bs1 + Dx, self.input_port.angle_deg)
...
class RouteToLine(__RouteToLine__):
    ...
    def define_points(self, pts):
        ...
        if line.is_parallel(line2):
            ...
            if line == line2:
                ...
                if (self.input_port.angle_deg - self.angle_out) % 360.0 < 0.1:
                    ...
                    pts = points_add_polar(pts, bs1 + bs2 + self.min_straight,
                                           self.input_port.angle_deg + 90.0)
                    pts = points_add_polar(pts, bs1 + bs2 + max(
                        self.min_straight, self.start_straight),
                                           self.input_port.angle_deg + 180.0)
                    pts = points_add_polar(pts, bs1 + bs2 + self.min_straight,
                ...
                if (self.input_port.angle_deg - self.angle_out) % 360.0 < 0.1:
                    ...
                    pts = points_add_polar(pts, self.start_straight + bs1,
                                           self.input_port.angle_deg)
                    A = angle_deg(
                        line.closest_point(self.input_port.position),
                        self.input_port.position)
                    pts = points_add_polar(pts, bs1 + bs2 + self.min_straight,
                                           A)
                    P2 = Coord2(pts[-1][0], pts[-1][1]).move_polar(
                        bs2 + self.min_straight, self.angle_out)
                    P = RouteToParallelLine(
            ...
            D = line.distance(self.input_port.position)
            angle_diff = abs((angle_deg(i, self.input_port.position) - self.
                              input_port.angle_deg + 180.0) % 360.0 - 180.0)
...
def RouteToManhattanRelativeToPosition(**kwargs):
    ...
    R = RouteToAngle(
        angle_out=angle_deg(direction), allow_unmatched_kwargs=True, **kwargs)
    ...
    if relation == '<':
        ...
        if out_coord >= position:
            return R
    return RouteToLine(
        line_point=pos_coord, angle_out=angle_deg(direction), **kwargs)
...
def RouteToEastAtMaxY(**kwargs):
    ...
    if R.out_ports[0].y <= y_position:
        ...
        return RouteToLine(
            line_point=(0.0, y_position), angle_out=0.0, **kwargs)
