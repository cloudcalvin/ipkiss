...
class RouteManhattanBasic(__RouteBasic__):
    ...
    def define_points(self, pts):
        ...
        T = Rotation(
            (0.0, 0.0), self.output_port.angle_deg + 180.0) + Translation(
        ...
        while 1:
            ...
            if count > 40:
                LOG.error(
                    "Exceeds allowed steps in RouteManhattanBasic: \nInput:%s\nOutput:%s\nShape:%s"
                    % (str(self.input_port), str(self.output_port),
                       str(S.points)))
            ...
            if a % 360.0 == 0.0:
                ...
                if abs(p[1]) < threshold and p[0] <= threshold:
                    ...
                    S += O.position
                    break
                elif p[0] + (bs1 + bs2 + self.end_straight +
                             s) < threshold and abs(p[1]) - (
                                 bs1 + bs2 + self.min_straight) > -threshold:
                    # sufficient space for S-bend
                    p = (-self.end_straight - bs2, p[1])
                    a = -sigp * 90.0
                elif p[0] + (2 * bs1 + 2 * bs2 + self.end_straight + s +
                             self.min_straight) < threshold:
                    # sufficient distance to move aside
                    p = (p[0] + s + bs1, p[1])
                    a = -sigp * 90.0
                elif abs(p[1]) - (2 * bs1 + 2 * bs2 +
                                  2 * self.min_straight) > -threshold:
                    p = (p[0] + s + bs1, p[1])
                    a = -sigp * 90.0
                else:
                    p = (p[0] + s + bs1, p[1])
...
class RouteManhattan(RouteManhattanBasic):
    ...
    def define_points(self, pts):
        ...
        S3 = RouteToManhattan(
            input_port=self.output_port,
            bend_radius=self.bend_radius,
            min_straight=self.min_straight,
            rounding_algorithm=self.rounding_algorithm)
        S3.start_straight = self.end_straight
        S2 = RouteManhattanBasic(
            input_port=S1.out_ports[0],
