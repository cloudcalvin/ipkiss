...
class RouteManhattanBasic(__RouteBasic__):
    ...
    def define_points(self, pts):
        ...
        T = Rotation((0.0, 0.0), self.output_port.angle_deg + 180.0) + Translation(
        ...
        while 1:
            ...
            if a % 360.0 == 0.0:
                ...
                if abs(p[1]) < threshold and p[0] <= threshold:
                    ...
                    p[0] + (bs1 + bs2 + self.end_straight + s) < threshold
                    and abs(p[1]) - (bs1 + bs2 + self.min_straight) > -threshold
                ):
                    ...
                    p[0]
                    + (2 * bs1 + 2 * bs2 + self.end_straight + s + self.min_straight)
                    < threshold
                ):
                    ...
                    abs(p[1]) - (2 * bs1 + 2 * bs2 + 2 * self.min_straight) > -threshold
                ):
                    p = (p[0] + s + bs1, p[1])
                    a = -sigp * 90.0
                else:
                ...
                if abs(p[1]) - (bs1 + bs2 + self.min_straight) > -threshold:
                    ...
                    p = (
                        min(
                            p[0] - s - bs1,
                            -self.end_straight - self.min_straight - 2 * bs1 - bs2,
                        ),
                        p[1],
                    )
                    a = -sigp * 90.0
                ...
                if ((-p[1] * siga) - (s + bs2) > -threshold) and (
                    ...
                    -p[0] - (self.end_straight + 2 * bs1 + bs2 + self.min_straight)
                    > -threshold
                ):
                    ...
                    p = (
                        p[0],
                        siga
                        * max(p[1] * siga + s + bs1, bs1 + bs2 + self.min_straight),
                    )
                    a = 0.0
                    ...
                    p = (
                        min(
                            p[0] - bs1 + bs2 + self.min_straight,
                            -2 * bs1 - bs2 - self.end_straight - self.min_straight,
                        ),
                        p[1] + siga * (bs2 + s),
                    )
                    # a remains the same
