...
class Vector(StrongPropertyInitializer, transformable.Transformable):
    ...
    position = Coord2Property(
        default=(0.0, 0.0), doc="Position of the port (coordinate)."
    )
    ...
    angle_rad = FunctionProperty(
        get_angle_rad,
        set_angle_rad,
        doc="The outward facing angle of the port in radians (stored in degrees by default, converted to radians if needed)",
    )
    ...
    angle_deg = FunctionProperty(
        get_angle_deg, set_angle_deg, doc="The outward facing angle of the port."
    )
    angle = angle_deg
    ...
    def __getitem__(self, key):
        ...
        if key == 1:
            ...
            raise IndexError(
                "Vector supports only subscription[0] and [1], not " + str(key)
            )
...
def vector_match_transform(vector1, vector2):
    ...
    return Translation(vector2.position - vector1.position) + Rotation(
        vector2.position, vector2.angle_deg - vector1.angle_deg + 180.0
    )
...
class Vector3(Vector):
    ...
    inclination_rad = FunctionProperty(
        get_inclination_rad,
        set_inclination_rad,
        doc="The outward facing angle of the port relative to the zenit, in radians (stored in degrees by default, converted to radians if needed)",
    )
    ...
    inclination_deg = FunctionProperty(
        get_inclination_deg,
        set_inclination_deg,
        doc="The outward facing angle of the port relative to the zenit.",
    )
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and (self.inclination_deg == other.inclination_deg)
        )

    def __ne__(self, other):
        return (
            self.position != other.position
            or (self.angle_deg != other.angle_deg)
            or (self.inclination_deg != other.inclination_deg)
        )

    def __repr__(self):
        return "<Vector3 (%f, %f), i=%f, a=%f>" % (
            self.x,
            self.y,
