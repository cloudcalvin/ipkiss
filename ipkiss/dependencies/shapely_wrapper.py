...
def shapely_geom_to_shape(g):
    ...
    if g.is_empty:
        ...
        if not g.exterior.is_ring:
            raise Exception(
                "Unexpected case : exterior geometry expected to be a Shapely linear ring."
            )
        if len(g.interiors) == 0:
            ...
            for interior in g.interiors:
                ...
                if interior.is_ring:
                    ...
                    raise Exception(
                        "Unexpected case : interior geometry expected to be a Shapely linear ring."
                    )
...
class ShapelyPolygonCollection(StrongPropertyInitializer):
    ...
    def fabricate_offspring(self, georep):
        return PolygonCollection(size_info=self.size_info, georep=georep)
