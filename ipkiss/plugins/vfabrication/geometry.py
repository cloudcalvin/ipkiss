...
class __ProcessSuperpositionMaterialStackGeometry2DPolygons__(CartesianGeometry2D):
    ...
    processes = ListProperty(
        required=True,
        doc="list of all the processes (in the same order as the columns are listed in the mapping table)",
    )
    mapping_definition = ListProperty(
        required=True,
        doc="list with tuples (processes_present, processes_not_present, material_stack) / material_stack",
    )
    process_polygons = DictProperty(
        required=True,
        doc="dictionary : key : process / value: object of type LayerShapelyPolygons",
    )
    material_stacks_shapely_polygons = DefinitionProperty(
        doc="contains the resulting geometry represented as tuple (material_stack_id, shapely_polygon)"
    )
    material_stacks_shapes = DefinitionProperty(
        doc="contains the resulting geometry represented as tuple (material_stack_id, shape)"
    )
    ...
    def __get_mapping_internal_format__(self):
        ...
        for pflags, mstack in self.mapping_definition:
            ...
            process_to_material_stack_map.append(
                (tuple(true_list), tuple(false_list), mstack)
            )
        return process_to_material_stack_map

    def define_canvas_size(self):
        canvas_size = (
            int(numpy.ceil(self.size_info.width / self.grid)),
    ...
    def define_material_stacks_shapely_polygons(self):
        ...
        for (
            ...
            exclude_process_set,
            material_stack,
        ) in self.__get_mapping_internal_format__():
                ...
                for e_p in exclude_process_set:
                    ...
                    if self.save_debug_images:
                        process_polygon.save_to_image(
                            ...
                            % (debug_counter_line, debug_counter, e_p.extension)
                        )
                    debug_counter = debug_counter + 1
    ...
    def __debug_savefig_material_array__(
        self, material_array, count, count_progress, material_stack_id
    ):
        if self.save_debug_images:
            # -debug code-
            ...
            pyplot.savefig(
                str(count)
                + "_"
                + self.material_stack_factory[material_stack_id].name.replace(" ", "_")
                + "_"
                + str(count_progress)
                + "_MATERIAL_ARRAY.png"
            )
    ...
    def __debug_savefig_polygon__(
        self, polygon_points, count, count_progress, material_stack_id
    ):
        if self.save_debug_images:
            # -debug code-
            from .vfabrication import LayerShapelyPolygons
            ...
            debug_lbm = LayerShapelyPolygons(
                ...
                size_info=SizeInfo(
                    west=0, east=self.canvas_size[0], south=0, north=self.canvas_size[1]
                ),
            )
    ...
    def get_material_array(self):
        ...
        if self.__material_array__ is None:
            ...
            material_array = numpy.zeros(
                [self.canvas_width, self.canvas_height], dtype=numpy.int16
            )  # store the material id as 16-bit integer
            count = 0
            for (material_stack_id, mb) in mat_bm:
                ...
                if mb is None:
                    ...
                    pass
                    material_array = (
                        material_array + material_stack_id
                    )  # set whole matrix to background material stack
                else:
...
class ProcessSuperpositionVirtualFabrication3DGeometryPolygons(
    ...
    def get_size_z(self):
        ...
        material_stack_id = mat_bm[1][
            0
        ]  # take the second element of mat_bm, not the first as that is the canvas polygon
