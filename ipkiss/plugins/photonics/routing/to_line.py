    ...
    def define_points(self, pts):
        ...
        if angle == 0.0:
            pts = points_add_polar(
                pts, max(self.start_straight, self.end_straight), self.angle_out
            )
        elif angle == 180.0:
            bs1_90, bs2_90 = self.get_bend90_size()
            pts = points_add_polar(
                pts, self.start_straight + bs1_90, self.input_port.angle_deg
            )
            pts = points_add_polar(
                pts,
                self.min_straight + bs1_90 + bs2_90,
                self.input_port.angle_deg + 90.0,
            )
            pts = points_add_polar(
                pts, self.end_straight + bs2_90, self.input_port.angle_deg + 180.0
            )
        else:
            bs1, bs2 = self.get_bend_size(angle)
            pts = points_add_polar(
                pts, self.start_straight + bs1, self.input_port.angle_deg
            )
    ...
    def define_points(self, pts):
        ...
        if line == line2:
            ...
            angle = self.find_angle(
                D, self.max_s_bend_angle, 0.0, self.s_bend_angle_resolution
            )
...
class RouteToLine(__RouteToLine__):
    ...
    def define_points(self, pts):
        ...
        bs1, bs2 = self.get_bend_size(angle)
        min_dist = (
            bs1 + self.start_straight
        )  # compute from min_straight (equals min-straight if not explicitely overruled)
        ...
        if line.is_parallel(line2):
            ...
            if line == line2:
                ...
                if (self.input_port.angle_deg - self.angle_out) % 360.0 < 0.1:
                    ...
                    pts = points_add_polar(
                        pts,
                        bs1 + bs2 + max(self.min_straight, self.start_straight),
            ...
            angle_diff = abs(
                (
                    angle_deg(i, self.input_port.position)
            ...
            if (Di - min_dist) >= 0 and angle_diff < 0.1:
                if (
                    ...
                    pts.extend([P1, P2])
                    pts = points_add_polar(
                        pts, L2_2 + self.end_straight, self.angle_out
                    )
            else:
                pts.append(self.input_port.position)
                pts = points_add_polar(
...
def RouteToEastAtMaxY(**kwargs):
    ...
    if R.out_ports[0].y <= y_position:
        ...
        return RouteToLine(line_point=(0.0, y_position), angle_out=0.0, **kwargs)
...
def RouteToWestAtMaxY(**kwargs):
    ...
    if R.out_ports[0].y <= y_position:
        ...
        return RouteToLine(line_point=(0.0, y_position), angle_out=180.0, **kwargs)
...
def RouteToNorthAtMaxX(**kwargs):
    ...
    if R.out_ports[0].x <= x_position:
        ...
        return RouteToLine(line_point=(x_position, 0.0), angle_out=90.0, **kwargs)
