...
__all__ = ["SplitterTreeLeftToRight", "SplitterWithYSpacing"]
...
class SplitterTreeBasic(Structure):
    ...
    def define_connectors(self):
        raise NotImplementedException(
            "SplitterTreeBasic::define_connectors to be implemented by subclasses"
        )

    def define_patches(self):
        raise NotImplementedException(
            "SplitterTreeBasic::define_patches to be implemented by subclasses"
        )

    def define_splitter_transforms(self):
        raise NotImplementedException(
            "SplitterTreeBasic::define_splitter_transforms to be implemented by subclasses"
        )
...
class SplitterWithYSpacing(SplitterTreeBasic):
    ...
    branch_level = (1,)
    connectors_splitter_transforms = DefinitionProperty(
        fdef_name="define_connectors_splitter_transforms"
    )
    ...
    def define_connectors_splitter_transforms(self):
        ...
        connectors = [
            [
                RouteConnectorRounded(
                    RouteToEastAtY(input_port=P[0], y_position=-0.5 * self.y_spacing)
                ),
                RouteConnectorRounded(
                    RouteToEastAtY(input_port=P[1], y_position=0.5 * self.y_spacing)
                ),
            ]
        ]
...
class SplitterTreeLeftToRight(SplitterTreeBasic):
    ...
    y_spacing = PositiveNumberProperty(required=True)
    patches_connectors_splitter_transforms = DefinitionProperty(
        fdef_name="define_patches_connectors_splitter_transforms"
    )

    def define_patches_connectors_splitter_transforms(self):
        N = 2 ** self.branch_level
        L = (
            self.splitter.out_ports.size_info().east
            - self.splitter.in_ports.size_info().west
        )
        T = [Translation(-self.splitter.in_ports[0].position)]
        splitter_transforms = []
        splitter_transforms += T
        ...
        for i in range(self.branch_level):
            ...
            for t in T:
                ...
                c1 = RouteConnectorRounded(
                    RouteToEastAtY(input_port=P_out[0], y_position=P_in[0].y - 0.5 * Y)
                )
                c2 = RouteConnectorRounded(
                    RouteToEastAtY(input_port=P_out[1], y_position=P_in[0].y + 0.5 * Y)
                )
                C += [c1, c2]
                ...
                wp = (
                    abs(c2o.position.y - c1o.position.y)
                    + 0.5 * (c1o.wg_definition.wg_width + c2o.wg_definition.wg_width)
                    + c1o.wg_definition.trench_width
                    + c2o.wg_definition.trench_width
                )
                    ...
                    T2 += [
                        Translation(
                            coord2_match_position(
                                self.splitter.in_ports[0], c1.out_ports[0]
                            )
                        ),
                        Translation(
                            coord2_match_position(
                                self.splitter.in_ports[0], c2.out_ports[0]
                            )
                        ),
                    ]
