...
from picazzo.wg.taper.layout import (
    __WgElPortTaper__,
    WgElTaperLinear,
    WgElPortTaperLinear,
)
...
class WgElPortTaperExtended(__WgElPortTaper__):
    ...
    length = FloatProperty(
        allow_none=True,
        doc="Length of the taper. Not setting it will use the default values.",
    )
    ...
    def define_taper(self):
        ...
        # Case: special tapering structures ...
            ...
            hasraised
            and type(self.start_port.wg_definition) == RaisedWGFCWgElDefinition
        ) and (type(self.end_wg_def) == WGFCWgElDefinition):
            taper = RaisedWGFCToWGFCPortTaper(
                start_port=self.start_port,
                end_wg_def=self.end_wg_def,
                straight_extension=self.straight_extension,
            )
        elif (
            hasraised and type(self.start_port.wg_definition) == WGFCWgElDefinition
        ) and (type(self.end_wg_def) == RaisedWGFCWgElDefinition):
            ...
            taper = Translation(
                translation=self.start_position.move_polar_copy(
                    self.length, self.start_port.angle_deg
                )
            )(taper)
        elif (
            hasraised and type(self.start_port.wg_definition) == RaisedFCWgElDefinition
        ) and (type(self.end_wg_def) == WgElDefinition):
            return RaisedFCToWgElPortTaper(
                start_port=self.start_port,
                end_wg_def=self.end_wg_def,
                straight_extension=self.straight_extension,
            )
        elif (
            hasraised and type(self.start_port.wg_definition) == RaisedWgElDefinition
        ) and (type(self.end_wg_def) == WgElDefinition):
            taper = RaisedWgElToWgElPortTaper(
                start_port=self.start_port,
                end_wg_def=self.end_wg_def,
                straight_extension=self.straight_extension,
            )
            ...
            if type(self.end_wg_def) == RaisedFCWgElDefinition:
                ...
                raise Exception(
                    "No taper could be generated between between waveguide types %s and %s."
                    % (self.start_port.wg_definition, self.end_wg_def)
                )
        else:
            raise Exception(
                "No taper could be generated between between waveguide types %s and %s."
    ...
    def validate_properties(self):
        return (
            True
        )  # override the restriction that type(self.start_wg_def) should be equal to type(self.end_wg_def)


# FIXME: Backward compatibility. This module originally contained these classes
TECH = get_technology()
if hasattr(TECH.PROCESS, "RFC"):
    from ..wgdefs.raised.tapers import (
        RaisedFCToWgElPortTaper,
        RaisedWgElToWgElPortTaper,
        RaisedWGFCToWGFCPortTaper,
    )
