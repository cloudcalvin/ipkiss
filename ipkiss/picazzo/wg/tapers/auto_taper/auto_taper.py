...
class WgElPortTaperAuto(__WgElPortTaper__):
    ...
    length = NonNegativeNumberProperty(
        allow_none=True,
        doc="Length of the taper. Not setting it will use the default values.")
    taper_data_base = DefinitionProperty(
        default=TECH.WGDEF.AUTO_TAPER_DATA_BASE)
    ...
    def __get_taper_class_swapped__(self):
        ...
        taper_class = self.taper_data_base.get_taper_class(
            start_wg_def, end_wg_def)

        # try and swap:
        if taper_class is None:
            taper_class = self.taper_data_base.get_taper_class(
                end_wg_def, start_wg_def)
            swapped = True
    ...
    def define_taper(self):
        ...
        if swapped:
            ...
            start_port = OpticalPort(
                position=self.start_port.
                position,  # move the taper later down the road over the correct length
