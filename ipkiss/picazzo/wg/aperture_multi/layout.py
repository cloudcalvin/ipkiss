...
class DeepWgApertureMerged(__DeepWgApertureMulti__):
    ...
    def define_elements(self, elems):
        ...
        elems += hull_inversion(
            shapelist,
            self.aperture_wg_definition.trench_width,
        ...
        for S1 in shapelist:
            elems += Boundary(
                PPLayer(self.aperture_wg_definition.process, TECH.PURPOSE.LF.LINE), S1
            )
        return elems


def hull_inversion_segment(
    shape0, shape1, trench_width, stub_sharp_angles=True, process=TECH.PROCESS.WG
):
    S0 = shape0
    sh0 = shape0[1:2]
    A0 = S0.angles_deg()
    ...
    if lines_cross(S0[0], S0[1], S1[0], S1[1]):
        ...
        if lines_cross(S0[2], S0[1], S1[2], S1[1]):
            ...
            sh0 = Shape(
                [
                    I2,
                    es[1].move_polar_copy(0.015, astub + 180.0),
                    es[2].move_polar_copy(0.015, astub),
                    es[2].move_polar_copy(TECH.TECH.MINIMUM_SPACE, A0[0] + 180.0),
                ]
            )
            sh1 = Shape(
                [
                    ...
                    es[2].move_polar_copy(0.015, astub),
                    es[1].move_polar_copy(0.015, astub + 180.0),
                    es[1].move_polar_copy(TECH.TECH.MINIMUM_SPACE, A1[0] + 180.0),
                ]
            )
    ...
    if D < 1.5 * TECH.TECH.MINIMUM_SPACE:
        ...
        es = ShapeStub(
            original_shape=[S1[0], intersection(S1[0], S1[1], S0[0], S0[1]), S0[0]],
        ...
        if abs(t0) < 88.0 and abs(t1) < 88.0:
            ...
            if abs(t0) > abs(t1):
                a = (
                    sign(t0) * RAD2DEG * acos(TECH.TECH.MINIMUM_SPACE / D_points)
                    + A0[0]
                )
                T1 += [S0[1], S0[1].move_polar_copy(TECH.TECH.MINIMUM_SPACE, a), S1[1]]
            else:
                a = (
                    sign(t1) * RAD2DEG * acos(TECH.TECH.MINIMUM_SPACE / D_points)
                    + A1[0]
                )
