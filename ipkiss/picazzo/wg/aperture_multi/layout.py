...
__all__ = [
    "WgApertureMultiFrom1d", "WgApertureMultiFrom1dGeneric",
    "DeepWgApertureMerged", "DeepWgApertureMulti"
]
...
class __WgApertureMulti__(__WgAperture__):
    ...
    vectors = RestrictedProperty(
        restriction=RestrictList(RestrictType(Vector)), required=True)
    dummy_list = RestrictedProperty(
        restriction=RestrictList(RESTRICT_INT), default=[])
    actual_dummy_list = DefinitionProperty(
        fdef_name="define_actual_dummy_list")
    trans = DefinitionProperty(fdef_name="define_trans")

    def define_name(self):
        p = "".join(
            ["%f,%f,%f" % (v.x, v.y, v.angle_deg) for v in self.vectors])
        dl = "".join([str(i) for i in self.dummy_list])

        return "%s_V%d_%d" % (self.__name_prefix__, do_hash(p), do_hash(dl))
...
class WgApertureMultiFrom1dGeneric(__WgApertureMulti__):
    ...
    apertures_1d = RestrictedListProperty(
        allowed_types=[__WgAperture__], required=True)
    ...
    def define_name(self):
        ...
        dl = "".join([str(i) for i in self.dummy_list])
        return "%s_%d_V%d_%d" % (self.__name_prefix__, do_hash(n), do_hash(p),
                                 do_hash(dl))

    def define_elements(self, elems):
        for i in range(len(self.trans)):
            elems += SRef(self.apertures_1d[i], (0.0, 0.0),
                          self.trans[i]).flat_copy()
        return elems

    def define_ports(self, ports):
        if len(self.apertures_1d) != len(
                self.vectors
        ):  #FIXME -- maybe we should make a method "assess_correctness" in Structure, which is called after construction?
            raise AttributeError(
                "Length of vectors and apertures_1d list is not equal in in WgApertureMultiFrom1dGeneric"
            )
        for i in range(len(self.trans)):
            if not i in self.actual_dummy_list:
                ports += self.apertures_1d[i].ports.transform_copy(
                    self.trans[i])
        return ports
...
class WgApertureMultiFrom1d(WgApertureMultiFrom1dGeneric):
    ...
    def define_name(self):
        p = "".join(
            ["%f,%f,%f" % (v.x, v.y, v.angle_deg) for v in self.vectors])
        dl = "".join([str(i) for i in self.dummy_list])
        return "%s_%s_V%d_%d" % (self.__name_prefix__, self.aperture_1d.name,
                                 do_hash(p), do_hash(dl))
...
class __DeepWgApertureMulti__(__WgApertureMulti__):
    ...
    def define_aperture_1d(self):
        return DeepWgAperture(
            aperture_wg_definition=self.aperture_wg_definition,
...
class DeepWgApertureMerged(__DeepWgApertureMulti__):
    ...
    def define_elements(self, elems):
        ...
        S = Shape(
            ShapeWedge(
                begin_coord=(0.0, 0.0),
                end_coord=(self.taper_length, 0.0),
                begin_width=self.aperture_wg_definition.wg_width,
                end_width=self.wg_definition.wg_width))
        # dummy taper
        SD1 = Shape(
            ShapeWedge(
                begin_coord=(0.0, 0.0),
                end_coord=(0.5 * self.taper_length, 0.0),
                begin_width=self.aperture_wg_definition.wg_width,
                end_width=0.5 * self.wg_definition.wg_width +
                0.5 * self.aperture_wg_definition.wg_width))
        SD2 = Shape(
            ShapeWedge(
                begin_coord=(0.5 * self.taper_length, 0.0),
                end_coord=(self.taper_length, 0.0),
                begin_width=0.5 * self.wg_definition.wg_width +
                0.5 * self.aperture_wg_definition.wg_width,
                end_width=self.aperture_wg_definition.wg_width))
        S_dummy = Shape(
            [SD1[0], SD1[1], SD1[2], SD2[2], SD2[3], SD1[3]], closed=True)
        if sign < 0:
            S.v_mirror()
            S_dummy.v_mirror()
        start = True
        ...
        elems += hull_inversion(
            shapelist, self.aperture_wg_definition.trench_width,
            self.wg_definition.trench_width, sign, (True, False))
        for S1 in shapelist:
            elems += Boundary(
                PPLayer(self.aperture_wg_definition.process,
                        TECH.PURPOSE.LF.LINE), S1)
        return elems


def hull_inversion_segment(shape0,
                           shape1,
                           trench_width,
                           stub_sharp_angles=True,
    ...
    if lines_cross(S0[0], S0[1], S1[0], S1[1]):
        ...
        if lines_cross(S0[2], S0[1], S1[2], S1[1]):
            # end faces cross
            es = ShapeStub(
                original_shape=[S1[0], I, S0[0]],
                stub_width=TECH.TECH.MINIMUM_SPACE + 2 * 0.015)
            astub = angle_deg(es[1], es[2])
            I2 = intersection(S0[2], S0[1], S1[2], S1[1])
            T1 += I2

            sh0 = Shape([
                I2, es[1].move_polar_copy(0.015, astub + 180.0),
                es[2].move_polar_copy(0.015, astub), es[2].move_polar_copy(
                    TECH.TECH.MINIMUM_SPACE, A0[0] + 180.0)
            ])
            sh1 = Shape([
                I2, es[2].move_polar_copy(0.015, astub), es[1].move_polar_copy(
                    0.015, astub + 180.0), es[1].move_polar_copy(
                        TECH.TECH.MINIMUM_SPACE, A1[0] + 180.0)
            ])
            ...
            astub = angle_deg(es[1], es[2])
            sh0 = Shape([
                S0[1], es[1].move_polar_copy(0.015, astub + 180.0),
                es[2].move_polar_copy(0.015, astub), es[2].move_polar_copy(
                    TECH.TECH.MINIMUM_SPACE, A0[0] + 180.0)
            ])
            sh1 = Shape([
                S0[1], S1[1],
                es[2].move_polar_copy(0.015, astub), es[1].move_polar_copy(
                    0.015, astub + 180.0), es[1].move_polar_copy(
                        TECH.TECH.MINIMUM_SPACE, A1[0] + 180.0)
            ])
            ...
            astub = angle_deg(es[1], es[2])
            sh0 = Shape([
                S1[1], S0[1], es[1].move_polar_copy(0.015, astub + 180.0),
                es[2].move_polar_copy(0.015, astub), es[2].move_polar_copy(
                    TECH.TECH.MINIMUM_SPACE, A0[0] + 180.0)
            ])
            sh1 = Shape([
    ...
    if D < 1.5 * TECH.TECH.MINIMUM_SPACE:
        ...
        es = ShapeStub(
            original_shape=[
                S1[0], intersection(S1[0], S1[1], S0[0], S0[1]), S0[0]
            ],
            stub_width=max(D, TECH.TECH.MINIMUM_SPACE) + 2 * 0.015)
        astub = angle_deg(es[1], es[2])
        sh0 = Shape([
            S0[1], S1[1], es[1].move_polar_copy(0.015, astub + 180.0),
            es[2].move_polar_copy(0.015, astub), es[2].move_polar_copy(
                TECH.TECH.MINIMUM_SPACE, A0[0] + 180.0)
        ])
        sh1 = Shape([
            S1[1], S0[1], es[2].move_polar_copy(0.015, astub),
            es[1].move_polar_copy(0.015, astub + 180.0), es[1].move_polar_copy(
                TECH.TECH.MINIMUM_SPACE, A1[0] + 180.0)
        ])
        T1 += [S0[1], S1[1]]
        ...
        if abs(t0) < 88.0 and abs(t1) < 88.0:
            ...
            if abs(t0) > abs(t1):
                a = sign(t0) * RAD2DEG * acos(
                    TECH.TECH.MINIMUM_SPACE / D_points) + A0[0]
                T1 += [
                    S0[1], S0[1].move_polar_copy(TECH.TECH.MINIMUM_SPACE, a),
                    S1[1]
                ]
            else:
                a = sign(t1) * RAD2DEG * acos(
                    TECH.TECH.MINIMUM_SPACE / D_points) + A1[0]
                T1 += [
                    S0[1], S1[1].move_polar_copy(TECH.TECH.MINIMUM_SPACE, a),
                    S1[1]
                ]
        ...
        t0 = shape0.turns_deg()[1]
        if abs(t0) < 93.0:
            Tx += S0[1].move_polar_copy(0.5 * trench_width,
                                        A0[0] - sign(t0) * 85.0)
        ...
        if abs(t1) < 93.0:
            Tx += S1[1].move_polar_copy(0.5 * trench_width,
                                        A1[0] - sign(t1) * 85.0)
        Tx += S1[1]
        ...
        if stub_sharp_angles:
            T1 += ShapeStub(
                original_shape=Tx,
                stub_width=TECH.TECH.MINIMUM_SPACE,
...
def hull_inversion(taper_shapes,
                   in_trench_width,
                   out_trench_width,
                   orientation,
                   stub_sharp_angles=(True, True),
                   process=TECH.PROCESS.WG):
    ...
    (T1sh, el, sh0, sh1) = hull_inversion_segment(
        shape0, shape1, in_trench_width, stub_sharp_angles[0], process=process)
    T1 += [p0]
    T1 += T1sh.reversed()
    ...
    shape1 = Shape([p0, p1, p2])
    (T2sh, el, sh2, sh3) = hull_inversion_segment(
        shape0, shape1, out_trench_width, stub_sharp_angles[1])
    T2 += [p0]
    T2 += T2sh.reversed()
    ...
    for S1 in taper_shapes[1:]:
        ...
        (T1sh, el, sh0, sh1) = hull_inversion_segment(
            shape0, shape1, in_trench_width, stub_sharp_angles[0])
        T1 += T1sh
        elements += el
        ...
        elements += Boundary(
            PPLayer(process, TECH.PURPOSE.LF_AREA), T1 + T2.reversed())
        T1 = T1sh
        T2 = T2sh
