...
class WgElTaperDeepShallow(WgElTaperLinear):
    ...
    def define_deep_only_wg_def(self):
        return WgElDefinition(
            wg_width=self.start_wg_def.wg_width + self.start_wg_def.trench_width * 2,
            trench_width=self.end_wg_def.trench_width,
        )

    def define_elements(self, elems):
        se = (
            self.straight_extension[0],
            max(TECH.TECH.MINIMUM_LINE, self.straight_extension[1]),
        )
        ## WG
        deep_only_end_position = Coord2(
            self.start_position[0], self.start_position[1]
        ).move_polar(
            self.deep_only_length_val, angle_deg(self.end_position, self.start_position)
        )
    ...
    def define_ports(self, ports):
        ...
        ports += [
            ...
            OutOpticalPort(
                position=self.end_position, wg_definition=self.end_wg_def, angle=angle
            ),
        ]
        return ports
...
class WgElTaperShallowDeep(WgElTaperDeepShallow):
    ...
    def define_elements(self, elems):
        se = (
            max(self.straight_extension[0], TECH.TECH.MINIMUM_LINE),
            self.straight_extension[1],
        )

        ## WG
        sp = Coord2(self.end_position[0], self.end_position[1]).move_polar(
            self.deep_only_length_val, angle_deg(self.start_position, self.end_position)
        )
        taper_wg = WgElTaperLinear(
...
TECH.WGDEF.AUTO_TAPER_DATA_BASE.add(
    WGFCWgElDefinition, WgElDefinition, WGFCWgElToWgElPortTaper
)
...
TECH.WGDEF.AUTO_TAPER_DATA_BASE.add(
    ShallowWgElDefinition, WgElDefinition, __WgElPortTaperFromShallowAuto__
)
