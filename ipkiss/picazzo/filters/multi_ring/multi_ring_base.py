...
def MultiRingIdenticalWithChangedInAndOut(
    ring_middle, ring_in, ring_out, ring_transformations, **kwargs
):
    ...
    rings = (
        [ring_in]
        + [ring_middle for i in range(len(ring_transformations) - 2)]
        + [ring_out]
    )
    return MultiRing(rings=rings, ring_transformations=ring_transformations, **kwargs)
        ...
        for s in self.spacings:
            ring_transformations.append(
                ...
                + Translation(
                    (
                        0.0,
                        self.ring.straights[1]
                        + 2 * numpy.mean(self.ring.get_bend90_size())
                        + s,
                    )
                )
            )
...
def MultiRingIdenticalWithChangedInAndOutWithSpacings(
    ring_middle, ring_in, ring_out, spacings, flip_last=False
):
    rings = [ring_in] + [ring_middle for i in range(len(spacings) - 1)] + [ring_out]
    return MultiRingWithSpacings(rings=rings, spacings=spacings, flip_last=flip_last)


def MultiRingIdenticalWithChangedInAndOutWithSpacing(
    ring_middle, ring_in, ring_out, spacing, n_o_rings, flip_last=False
):
    spacings = [spacing for i in range(n_o_rings - 1)]
