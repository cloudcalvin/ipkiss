...
__all__ = [
    "Uniform2dGrating", "BiaxialUniform2dGrating", "SymmetricUniform2dGrating",
    "SymmetricUniformBiaxialLongOctagon2dGrating",
    "SymmetricUniformBiaxialRect2dGrating", "SymmetricUniformRect2dGrating",
    "AsymmetricUniform2dGrating", "AsymmetricUniformRect2dGrating",
    "ThreeportUniform2dGrating", "ThreeportUniformRect2dGrating",
    "UniformDodec2dGrating", "UniformLongOctagon2dGrating",
    "UniformRect2dGrating", "UniformSquare2dGrating"
]
...
class Uniform2dGrating(__UnitCell__, FiberCoupler2dGratingAuto):
    ...
    def __get_grating__(self):
        origin = (-0.5 * (self.n_o_periods - 1) * self.period,
                  -0.5 * (self.n_o_periods - 1) * self.period)
        return (GratingUniform(
            unit_cell=self.unit_cell,
            origin=origin,
            period=(self.period, self.period),
            n_o_periods=(self.n_o_periods, self.n_o_periods)), Rotation(
                rotation_center=(0.0, 0.0), rotation=45.0))
...
class BiaxialUniform2dGrating(__UnitCell__, FiberCoupler2dGratingAuto):
    ...
    def __get_grating__(self):
        origin = (-0.5 * (self.n_o_periods[0] - 1) * self.periods[0],
                  -0.5 * (self.n_o_periods[1] - 1) * self.periods[1])
        return (GratingUniform(
            unit_cell=self.unit_cell,
            origin=origin,
            period=self.periods,
            n_o_periods=self.n_o_periods), Rotation((0.0, 0.0), 45))
...
class UniformSquare2dGrating(__AutoUnitCell__, Uniform2dGrating):
    ...
    def define_unit_cell(self):
        ...
        return Structure(self.name + "_hp",
                         Boundary(
                             PPLayer(self.process, TECH.PURPOSE.DF.SQUARE), s))
    ...
    def define_elements(self, elems):
        ...
        elems += Boundary(
            PPLayer(self.process, TECH.PURPOSE.DF.TRENCH),
...
class UniformRect2dGrating(__AutoUnitCell__, BiaxialUniform2dGrating):
    ...
    def define_unit_cell(self):
        s = ShapeRectangle(
            center=(0.0, 0.0),
            box_size=(self.hole_diameters[0], self.hole_diameters[1]))
        s.transform(Rotation(rotation=45.0))
        return Structure(self.name + "_hpr",
                         Boundary(
                             PPLayer(self.process, TECH.PURPOSE.DF.SQUARE), s))
...
class SymmetricUniform2dGrating(__SymmetricSocket2dGrating__,
                                Uniform2dGrating):
...
class __AsymmetricSocket2dGrating__(__AutoSocket__):
    ...
    wg_lengths = RestrictedProperty(
        restriction=RestrictLen(2) & RestrictList(RESTRICT_POSITIVE),
        required=True)
    dev_angles = RestrictedProperty(
        restriction=RestrictLen(2) & RestrictList(RESTRICT_NUMBER),
        required=True)

    def __get_socket_and_pos__(self):
        ap = BroadWgSocket2dAsymmetric(
            wg_definitions=self.wg_definitions,
...
class AsymmetricUniform2dGrating(__AsymmetricSocket2dGrating__,
                                 Uniform2dGrating):
    """abstract base class for uniform 2D grating with input and output waveguides, 1 symmetry plane
...
class ThreeportUniform2dGrating(__AutoSocket__, Uniform2dGrating):
    ...
    wg_lengths = RestrictedProperty(
        ...
        required=True)
    dev_angle = AngleProperty(required=True)
    truncate_periods_3port = IntProperty(
        default=0, restriction=RESTRICT_NONNEGATIVE)

    def __get_socket_and_pos__(self):
        ap = BroadWgSocket3Port(
            wg_definitions=self.wg_definitions,
            wg_lengths=self.wg_lengths,
            dev_angle=self.dev_angle,
    ...
    def __get_grating__(self):
        ...
        positions = [
            Coord2(-0.5 * (self.n_o_periods - 1) * pitch + t * pitch, 0.0)
            for t in range((
                self.truncate_periods_3port + 1) / 2, self.n_o_periods)
        ]
        for i in range(1, self.n_o_periods):
            N = self.n_o_periods - i + max(0,
                                           i / 2 - self.truncate_periods_3port)
            positions += [
                Coord2(-0.5 *
                       (self.n_o_periods - 1 - i % 2) * pitch + t * pitch,
                       0.5 * i * pitch)
                for t in range((self.truncate_periods_3port + 1 - i % 2
                                ) / 2, self.n_o_periods - (i + 1) / 2)
            ]
            positions += [
                Coord2(-0.5 *
                       (self.n_o_periods - 1 - i % 2) * pitch + t * pitch,
        ...
        return (GratingUnitCell(unit_cell=self.unit_cell, positions=positions),
                None)


class SymmetricUniformRect2dGrating(__SymmetricSocket2dGrating__,
                                    UniformDodec2dGrating):
    """ uniform 2D grating with rectangular unit cell and 2 symmetry planes """


class SymmetricUniformBiaxialRect2dGrating(__SymmetricSocket2dGrating__,
...
class SymmetricUniformBiaxialLongOctagon2dGrating(__SymmetricSocket2dGrating__,
...
class AsymmetricUniformRect2dGrating(AsymmetricUniform2dGrating,
                                     UniformDodec2dGrating):
    """ uniform 2D grating with rectangular unit cell and 1 symmetry plane """


class ThreeportUniformRect2dGrating(ThreeportUniform2dGrating,
                                    UniformDodec2dGrating):
