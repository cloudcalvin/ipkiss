...
class InvertedTaper(FiberCoupler):
    ...
    wg_def_start = WaveguideDefProperty(
        default=WgElDefinition(
    ...
    def define_elements(self, elems):
        ...
        wg_def_stop = WgElDefinition(
            ...
            trench_width=0.5 * (self.y_spacing - self.wg_def_end.wg_width) - 1.0,
        )
        ...
        if self.extension > 0:
            ...
            elems += wg_def(
                [(self.tip_offset - self.extension, 0.0), (self.tip_offset, 0.0)]
            )
        ...
        for i in range(self.assist_lines):
            elems += Path(
                ...
                [
                    (
                        self.tip_length + self.tip_offset,
                        0.0
                        + (
                            0.5 * self.tip_start_width
                            + (i + 1) * (self.assist_width + self.assist_spacing)
                    ...
                    (
                        self.tip_offset - self.extension,
                        0.0
                        + (
                            0.5 * self.tip_width
                            + (i + 1) * (self.assist_width + self.assist_spacing)
...
class NitrideInvertedTaper(InvertedTaper):
    ...
    def define_elements(self, elems):
        ...
        elems += Line(
            ...
            (0.0, 0.0),
            (self.nitride_only_length - self.extension, 0.0),
            self.y_spacing,
        )
        # NT
        elems += Line(
            PPLayer(self.nt_process, TECH.PURPOSE.LF.LINE),
            (0.0, 0.0),
...
class InvertedTaperShallow(InvertedTaper):
    ...
    wg_def_sh_start = WaveguideDefProperty(
        default=WgElDefinition(wg_width=1.0, trench_width=TECH.WG.TRENCH_WIDTH)
    )
    wg_def_sh_end = WaveguideDefProperty(
        default=WgElDefinition(wg_width=0.080, trench_width=TECH.WG.TRENCH_WIDTH)
    )
    ...
    def define_elements(self, elems):
        ...
        wg_def_sh_end = WgElDefinition(
            ...
            trench_width=0.5 * (self.y_spacing - self.wg_def_sh_end.wg_width) - 1.0,
        )
        elems += WgElTaperLinear(
    ...
    def define_ports(self, ports):
        ...
        ports += [
            OpticalPort(
                position=(
                    self.tip_length + self.tip_offset + self.shallow_tip_length,
                    0.0,
                ),
                angle=0.0,
                wg_definition=wg_def,
...
class NitrideInvertedTaperShallow(InvertedTaperShallow):
    ...
    def define_elements(self, elems):
        # WG
        elems += Line(
            PPLayer(self.deep_process, TECH.PURPOSE.LF_AREA),
            (0.0, 0.0),
            (self.nitride_only_length - self.extension, 0.0),
            self.y_spacing,
        )
        ...
        elems += Line(
            ...
            (0.0, 0.0),
            (
                self.nitride_only_length
                + self.tip_length
                + self.shallow_tip_length
                + 50.0,
                0.0,
            ),
            self.nitride_width,
