...
__all__ = [
    "GratingGeneric", "GratingUnitCell", "GratingUniform", "GratingUniformLine"
]
...
class __AutoUnitCell__(__UnitCell__):
    ...
    def define_unit_cell(self):
        raise NotImplementedException(
            "To be implemented by subclass of __AutoUnitCell__ :: define_unit_cell"
        )
...
class GratingGeneric(Structure):
    ...
    cells_positions = RestrictedProperty(
...
class GratingUnitCell(__UnitCell__, GratingGeneric):
    ...
    positions = RestrictedProperty(
        restriction=RestrictTypeList(Coord2), required=True)
...
class GratingUniform(GratingUnitCell):
    ...
    def define_name(self):
        L = []
        L.extend(
            [str(int(self.origin[0] * 1000)),
             str(int(self.origin[0] * 1000))])
        L.extend(
            [str(int(self.period[0] * 1000)),
             str(int(self.period[0] * 1000))])
        L.extend([str(self.n_o_periods[0]), str(self.n_o_periods[0])])
        return "%s_%s_%d" % (self.__name_prefix__, self.unit_cell.name,
                             do_hash("_".join(L)))

    def define_elements(self, elems):
        elems += SoftRotationARef(self.unit_cell, self.origin, self.period,
                                  self.n_o_periods)
        return elems
...
def GratingUniformLine(line_width,
    ...
    unit_cell = Structure(
        "line" + str(int(line_width * 1000)) +
        "_" + str(int(line_length * 1000)) + "_" + str(
            PPLayer(process, purpose)).replace(" ", "_").replace("-", "_"),
        Rectangle(
            PPLayer(process, purpose), (-0.5 * line_width, 0.0),
