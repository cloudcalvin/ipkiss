...
__all__ = ["GratingGeneric", "GratingUnitCell", "GratingUniform", "GratingUniformLine"]
...
class GratingGeneric(Structure):
    ...
    cells_positions = RestrictedProperty(
        restriction=RestrictType(dict), required=True
...
class GratingUniform(GratingUnitCell):
    ...
    def define_elements(self, elems):
        elems += SoftRotationARef(
            self.unit_cell, self.origin, self.period, self.n_o_periods
        )
        return elems


def GratingUniformLine(
    line_width,
    line_length,
    period,
    n_o_periods,
    ...
    unit_cell = Structure(
        "line"
        + str(int(line_width * 1000))
        + "_"
        + str(int(line_length * 1000))
        + "_"
        + str(PPLayer(process, purpose)).replace(" ", "_").replace("-", "_"),
        Rectangle(
            PPLayer(process, purpose),
            (-0.5 * line_width, 0.0),
