...
class WgSocket2dFrom1d(WgSocket2d):
    ...
    def define_ports(self, ports):
        return self.socket_1d.ports.transform_copy(
            self.trans1
        ) + self.socket_1d.ports.transform_copy(self.trans2)
...
class BroadWgSocket2dAsymmetric(WgSocket2d):
    ...
    wg_lengths = RestrictedProperty(
        restriction=RestrictLen(2) & RestrictList(RESTRICT_POSITIVE), required=True
    )
    dev_angles = RestrictedProperty(
        restriction=RestrictLen(2) & RestrictList(RESTRICT_NUMBER), required=True
    )
    ...
    def define_p_tr(self):
        ...
        return Coord2(0.0, 0.0).move_polar(
            0.5 * ww + self.wg_lengths[0], 45 - self.dev_angles[0]
        )
    ...
    def define_p_tl(self):
        ww = max(self.wg_definitions[0].wg_width, self.wg_definitions[1].wg_width)
        return Coord2(0.0, 0.0).move_polar(
            0.5 * ww + self.wg_lengths[1], 135 + self.dev_angles[1]
        )
    ...
    def define_elements(self, elems):
        ...
        sh_wg = (
            ...
            + ShapeStub(
                original_shape=[
                    L_br[i_er],
                    intersection(L_br[i_sr], L_br[i_er], L_bl[i_sl], L_bl[i_el]),
                    L_bl[i_el],
                ],
                stub_width=TECH.TECH.MINIMUM_LINE,
            )
            + ShapeStub(
                original_shape=[
                    L_bl[i_er],
                    intersection(L_bl[i_sr], L_bl[i_er], L_tl[i_sl], L_tl[i_el]),
                    L_tl[i_el],
                ],
                stub_width=TECH.TECH.MINIMUM_LINE,
            )
            + ShapeStub(
                original_shape=[
                    L_tl[i_er],
                    intersection(L_tl[i_sr], L_tl[i_er], L_tr[i_sl], L_tr[i_el]),
                    L_tr[i_el],
                ],
                stub_width=TECH.TECH.MINIMUM_LINE,
            )
        ...
        sh_tr = Shape(
            [L_tr[i_er], T_tr[i_er], T_br[i_el], L_br[i_el]]
        ...
        elems += Boundary(
            PPLayer(self.wg_definitions[0].process, TECH.PURPOSE.LF.LINE), sh_wg
        )
        elems += Boundary(
            PPLayer(self.wg_definitions[0].process, TECH.PURPOSE.LF_AREA), sh_tr
        )
        return elems
...
class BroadWgSocket3Port(WgSocket2d):
    ...
    def define_p_tr(self):
        ...
        return Coord2(0.0, 0.0).move_polar(
            0.5 * ww + self.wg_lengths[0], 45 - self.dev_angle
        )
    ...
    def define_p_l(self):
        ww = self.wg_definitions[0].wg_width
        return Coord2(0.0, 0.0).move_polar(
            0.5 * sqrt(0.5) * ww + self.wg_lengths[1] + self.port3_offset, 180.0
        )
    ...
    def define_elements(self, elems):
        ...
        sh_wg = (
            ...
            + ShapeStub(
                original_shape=[
                    L_br[i_er],
                    intersection(L_br[i_sr], L_br[i_er], L_tr[i_sr], L_tr[i_er]),
                    L_l[i_el].move_copy((self.wg_lengths[1], 0.0)),
                    L_l[i_el],
                ],
                stub_width=TECH.TECH.MINIMUM_LINE,
            ...
            + ShapeStub(
                original_shape=[
                    L_l[i_er],
                    L_l[i_er].move_copy((self.wg_lengths[1], 0.0)),
                    intersection(L_tr[i_el], L_tr[i_sl], L_br[i_el], L_br[i_sl]),
                    L_tr[i_el],
                ],
                stub_width=TECH.TECH.MINIMUM_LINE,
            )
    ...
    def define_ports(self, ports):
        ports += [
            ...
            OpticalPort(
                position=self.p_l, wg_definition=self.wg_definitions[1], angle=180.0
            ),
        ]
        return ports
