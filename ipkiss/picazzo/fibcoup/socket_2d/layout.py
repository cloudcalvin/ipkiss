...
__all__ = [
    "WgSocket2dFrom1d", "BroadWgSocket2d", "BroadWgSocket2dAsymmetric",
    "BroadWgSocket3Port"
]
...
class WgSocket2dFrom1d(WgSocket2d):
    ...
    def define_trans1(self):
        return Translation(self.position_1d) + Rotation(
            (0.0, 0.0), 45.0 - self.dev_angle)
    ...
    def define_ports(self, ports):
        return self.socket_1d.ports.transform_copy(
            self.trans1) + self.socket_1d.ports.transform_copy(self.trans2)
...
class BroadWgSocket2dAsymmetric(WgSocket2d):
    ...
    def define_p_tr(self):
        ww = max(self.wg_definitions[0].wg_width,
                 self.wg_definitions[1].wg_width)
        return Coord2(0.0, 0.0).move_polar(0.5 * ww + self.wg_lengths[0],
                                           45 - self.dev_angles[0])
    ...
    def define_p_tl(self):
        ww = max(self.wg_definitions[0].wg_width,
                 self.wg_definitions[1].wg_width)
        return Coord2(0.0, 0.0).move_polar(0.5 * ww + self.wg_lengths[1],
                                           135 + self.dev_angles[1])
    ...
    def define_elements(self, elems):
        ...
        L_tr = ShapePath(
            original_shape=[(0.0, 0.0), self.p_tr], path_width=ww0)
        L_br = ShapePath(
            original_shape=[(0.0, 0.0), self.p_br], path_width=ww0)
        L_tl = ShapePath(
            original_shape=[(0.0, 0.0), self.p_tl], path_width=ww1)
        L_bl = ShapePath(
            original_shape=[(0.0, 0.0), self.p_bl], path_width=ww1)
        ...
        sh_wg = (ShapeStub(
            ...
            stub_width=TECH.TECH.MINIMUM_LINE) + ShapeStub(
                original_shape=[
                    L_br[i_er],
        ...
        # path shape from center to OpticalPort
        T_tr = ShapePath(
            original_shape=[(0.0, 0.0), self.p_tr], path_width=ww0 + 2 * tw0)
        T_br = ShapePath(
            original_shape=[(0.0, 0.0), self.p_br], path_width=ww0 + 2 * tw0)
        T_tl = ShapePath(
            original_shape=[(0.0, 0.0), self.p_tl], path_width=ww1 + 2 * tw1)
        T_bl = ShapePath(
            original_shape=[(0.0, 0.0), self.p_bl], path_width=ww1 + 2 * tw1)
    ...
    def define_ports(self, ports):
        ports += [
            OpticalPort(
                ...
                angle=45.0 - self.dev_angles[0]),
            OpticalPort(
                position=self.p_tl,
                wg_definition=self.wg_definitions[1],
                angle=135.0 + self.dev_angles[1]),
            OpticalPort(
                position=self.p_bl,
                wg_definition=self.wg_definitions[1],
                angle=225.0 - self.dev_angles[1]),
            OpticalPort(
                position=self.p_br,
                wg_definition=self.wg_definitions[0],
                angle=315.0 + self.dev_angles[0])
        ]
        return ports
...
def BroadWgSocket2d(wg_definition, wg_length, dev_angle, **kwargs):
    ...
    return BroadWgSocket2dAsymmetric(
        wg_definitions=(wg_definition, wg_definition),
        wg_lengths=(wg_length, wg_length),
        dev_angles=(dev_angle, dev_angle),
        **kwargs)
...
class BroadWgSocket3Port(WgSocket2d):
    ...
    def define_p_l(self):
        ww = self.wg_definitions[0].wg_width
        return Coord2(0.0, 0.0).move_polar(
            0.5 * sqrt(0.5) * ww + self.wg_lengths[1] + self.port3_offset,
            180.0)
    ...
    def define_elements(self, elems):
        ...
        sh_wg = (ShapeStub(
            ...
            stub_width=TECH.TECH.MINIMUM_LINE) + ShapeStub(
                ...
                stub_width=TECH.TECH.MINIMUM_LINE,
                only_sharp_angles=True) + ShapeStub(
                    original_shape=[
                        L_l[i_er], L_l[i_er].move_copy(
                            (self.wg_lengths[1], 0.0)),
        ...
        # path shape from center to OpticalPort
        T_tr = ShapePath(
            original_shape=[(0.0, 0.0), self.p_tr],
            path_width=ww0 + 2 * self.wg_definitions[0].trench_width)
        T_br = ShapePath(
            original_shape=[(0.0, 0.0), self.p_br],
            path_width=ww0 + 2 * self.wg_definitions[0].trench_width)
        T_l = ShapePath(
            original_shape=[(0.0, 0.0), self.p_l],
            path_width=ww1 + 2 * self.wg_definitions[0].trench_width)

        sh_tr = Shape([
            L_tr[i_er], T_tr[i_er], T_br[i_el], L_br[i_el], L_br[i_er],
            T_br[i_er], (0.0, T_br[i_er].y), T_l[i_el], L_l[i_el], L_l[i_er],
            T_l[i_er], (0.0, T_tr[i_el].y), T_tr[i_el], L_tr[i_el]
        ])
        sh_tr.close()
