        ...
        for line in lines:
            ...
            for p in patterns:
                ...
                if len(q) == 1:
                    ...
                    if n_o_periods < 0:
                        LOG.error(
                            "Wrong syntax in repetions (invalid number of periods)"
                        )
                        raise AttributeError
                    if period <= 0.0:
                        LOG.error(
                            "Wrong syntax in repetions (content of period is empty)"
                        )
                        raise SystemExit
                    for char in content:
                        if char in self.cells:
                    ...
                    LOG.error(
                        "Wrong syntax in repetions (do not use , or / in patterns)"
                    )
                    raise AttributeError
            maxlen = max([t, maxlen])
            ypos += self.pitches[1]
...
class __PhCAutoMap__(object):
    ...
    def define_map(self):
        raise NotImplementedException(
            "Function 'define_maps' to be implemented by subclass of __PhCAutoMap__"
        )
