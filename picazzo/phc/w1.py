...
class GenericW1WaveguideSection(__GenericW1Waveguide__):
    ...
    def define_name(self):
        ...
        port_width = (
            sqrt(3.0) * self.lattice_pitches[1]
            if self.port_width is None
            else self.port_width
        )
    ...
    def define_ports_coordinates(self):
        ...
        port_row = (
            (len(self.unit_cells) - 1) / 2 if self.port_row is None else self.port_row
        )
        port_col = self.port_offset / self.pitches[0]

        return [
            ((-port_col, -port_row), -180, wg_def),
...
class W1Waveguide(__W1__):
    ...
    def define_name(self):
        return "%s_A%d_D%d_C%d_L%d_DC%d_%s_%s" % (
            self.__name_prefix__,
            self.pitch * 1000,
            self.diameter * 1000,
...
class W1WaveguideWithInlineCavity(__W1__):
    ...
    def define_hole_sizes_map(self):
        ...
        maplist.append(
            "/" + str(n_core) + ", Z/ " + cav_str + "/" + str(n_core) + ",Z /"
        )
...
class W1WaveguideWithAllpass(__W1__):
    ...
    def define_hole_sizes_map(self):
        ...
        maplist.append(
            "/"
            + str(int(ceil(self.n_o_periods / 2 - 1)))
            + ",A /Z /"
            + str(self.n_o_periods - int(ceil(self.n_o_periods / 2)))
            + ",A /"
        )
        maplist.append(" /" + str(self.n_o_periods - 1) + ",A /")
        maplist.append("/" + str(self.n_o_periods) + ",A /")
