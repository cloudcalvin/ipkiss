    ...
    def __get_grating__(self):
        ...
        return (
            GratingUniform(
                ...
                n_o_periods=(self.n_o_periods, self.n_o_periods),
            ),
...
class BiaxialUniform2dGrating(__UnitCell__, FiberCoupler2dGratingAuto):
    ...
    def __get_grating__(self):
        origin = (
            -0.5 * (self.n_o_periods[0] - 1) * self.periods[0],
...
class UniformSquare2dGrating(__AutoUnitCell__, Uniform2dGrating):
    ...
    def define_elements(self, elems):
        ...
        elems += Boundary(
            PPLayer(self.process, TECH.PURPOSE.DF.TRENCH), coordinates=my_shape
        )
        return elems
...
class UniformRect2dGrating(__AutoUnitCell__, BiaxialUniform2dGrating):
    ...
    def define_unit_cell(self):
        s = ShapeRectangle(
            center=(0.0, 0.0), box_size=(self.hole_diameters[0], self.hole_diameters[1])
        )
...
class ThreeportUniform2dGrating(__AutoSocket__, Uniform2dGrating):
    ...
    def __get_grating__(self):
        ...
        positions = [
            ...
            for t in range((self.truncate_periods_3port + 1) / 2, self.n_o_periods)
        ]
        for i in range(1, self.n_o_periods):
            N = self.n_o_periods - i + max(0, i / 2 - self.truncate_periods_3port)
            positions += [
                Coord2(
                    -0.5 * (self.n_o_periods - 1 - i % 2) * pitch + t * pitch,
                    0.5 * i * pitch,
                )
                for t in range(
                    (self.truncate_periods_3port + 1 - i % 2) / 2,
                    self.n_o_periods - (i + 1) / 2,
                )
            ]
            positions += [
                Coord2(
...
class SymmetricUniformRect2dGrating(
    __SymmetricSocket2dGrating__, UniformDodec2dGrating
):
    """ uniform 2D grating with rectangular unit cell and 2 symmetry planes """


class SymmetricUniformBiaxialRect2dGrating(
    __SymmetricSocket2dGrating__, UniformRect2dGrating
):
...
class SymmetricUniformBiaxialLongOctagon2dGrating(
    __SymmetricSocket2dGrating__, UniformLongOctagon2dGrating
):
    """ biaxial uniform 2D grating with rectangula unit cell and 2 symmetry planes """
