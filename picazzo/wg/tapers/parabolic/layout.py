...
class WgElTaperParabolic(__WgElTaper__):
    ...
    def __get_taper_shape__(
        self, start_position, end_position, start_window, end_window, se
    ):
        sp = start_position
        ep = end_position
        dist = distance(ep, sp)
        ...
        sp = (
            start_position[0] + (sinangle * (avg_offset_start)),
            start_position[1] + (cosangle * (avg_offset_start)),
        )
        ep = (
            end_position[0] + (sinangle * (avg_offset_end)),
        ...
        if se[0]:
            s1 += [
                (
                    sp[0]
                    + sinangle * sw / 2.0
                    - cosangle * (se[0] + TECH.WG.OVERLAP_TRENCH),
                    sp[1]
                    - cosangle * sw / 2.0
                    - sinangle * (se[0] + TECH.WG.OVERLAP_TRENCH),
                ),
                (
                    sp[0]
                    - sinangle * sw / 2.0
        ...
        if se[1]:
            s1 += [
                (
                    ep[0]
                    - sinangle * ew / 2.0
                    + cosangle * (se[1] + TECH.WG.OVERLAP_TRENCH),
                    ep[1]
                    + cosangle * ew / 2.0
                    + sinangle * (se[1] + TECH.WG.OVERLAP_TRENCH),
                ),
                (
                    ep[0]
                    + sinangle * ew / 2.0
