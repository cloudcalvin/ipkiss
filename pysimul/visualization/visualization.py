...
class SimulationVolumeVisualization2D(__SimulationVolumeVisualization__):
    simulation_volume = RestrictedProperty(
        required=True,
        restriction=RestrictType(SimulationVolume2D),
        doc="The simulation volume that we want to visualize",
    )
    ...
    def visualize(self, aspect_ratio_equal=True):
        ...
        for (material_stack_id, mb) in geom.material_stacks_shapely_polygons:
            color = geom.material_stack_factory[
                material_stack_id
            ].display_style.color.html_string()
            if (not (mb is None)) and (not mb.georep.is_empty):
                for polygon in mb.georep_list:
                    if isinstance(polygon, Polygon):
                        ...
                        LOG.error(
                            "An element of type %s will not be plotted." % type(polygon)
                        )
            count = count + 1.0
...
class SimulationVolumeVisualization3D(SimulationVolumeVisualization2D):
    def make_povray_file(self, camera_pos, look_at=(0, 0, 1), z_extrusion_factor=1.0):
        ...
        from dependencies.povray_wrapper import (
            PovrayFile,
            Background,
            Camera,
            LightSource,
        ...
        for (material_stack_id, shape) in geom.material_stacks_shapes[1:]:
            ...
            if len(shape.points) > 0 and solid_height > 0:
                ...
                print(
                    "Writing prism with %i points for %s (solid height : %f) in %s"
                    % (
                        len(shape.points),
                        geom.material_stack_factory[material_stack_id].name,
                        geom.material_stack_factory[material_stack_id].solid_height,
                        color,
                    )
                )
