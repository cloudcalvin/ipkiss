...
class ShapeEllipseArc(Shape):
    ...
    def define_points(self, pts):
        ...
        pts = numpy.column_stack((numpy.cos(angles), numpy.sin(angles))) * numpy.array(
            [(h_radius, v_radius)]
    ...
    def is_empty(self):
        return (
            ...
            or self.box_size[0] == 0.0
            or self.box_size[1] == 0.0
        )
...
class ShapeBend(ShapeArc):
    ...
    def move(self, position):
        self.start_point = Coord2(
            self.start_point[0] + position[0], self.start_point[1] + position[1]
        )
        return self


def ShapeBendRelative(
    start_point=(0.0, 0.0),
    radius=1.0,
    input_angle=0.0,
    angle_amount=90.0,
    ...
    def define_points(self, pts):
        ...
        if self.radius <= 0:
            ...
            if (self.radius == self.box_size[0] / 2.0) and (
                ...
                pts += ShapeCircle(
                    center=self.center, radius=self.radius, angle_step=self.angle_step
                )
            elif self.radius == self.box_size[0] / 2.0:
                pts += ShapeArc(
                    center=(cx, cy + dy - self.radius),
                    radius=self.radius,
                    start_angle=0.0,
                    end_angle=180.0,
                ...
                pts += ShapeArc(
                    center=(cx + dx - self.radius, cy + dy - self.radius),
                    radius=self.radius,
                    start_angle=0.0,
                    end_angle=90.0,
                    angle_step=self.angle_step,
    ...
    def define_points(self, pts):
        ...
        # radius = zero: part of rectangle
            ...
            for a in arange(self.start_angle, self.end_angle + 45.0, as_sign * 90.0):
                pts += [
                    (
                        cx + sign(math.cos(DEG2RAD * a)) * dx,
                        cy + sign(math.sin(DEG2RAD * a)) * dy,
                    )
                ]
        # arbitrary
        else:
            for a in numpy.arange(
                self.start_angle, self.end_angle + 45.0, as_sign * 90.0
            ):
                start = max(self.start_angle, a - a % 90.0 + 45.0 - as_sign * 45.0)
                end = min(self.end_angle, a - a % 90.0 + 45.0 + as_sign * 45.0)
                pts += ShapeArc(
                    center=(
                        cx + numpy.sign(math.cos(DEG2RAD * a)) * (dx - self.radius),
                        cy + numpy.sign(math.sin(DEG2RAD * a)) * (dy - self.radius),
                    ),
                    radius=self.radius,
                    start_angle=start,
                    end_angle=end,
...
class ShapeWedge(Shape):
    ...
    def move(self, position):
        self.begin_coord = (
            self.begin_coord[0] + position[0],
            self.begin_coord[1] + position[1],
        )
        self.end_coord = (
            self.end_coord[0] + position[0],
            self.end_coord[1] + position[1],
        )
        return self
...
class ShapeRadialWedge(Shape):
    ...
    def define_points(self, pts):
        ...
        bc = (
            self.center[0] + self.inner_radius * math.cos(self.angle * DEG2RAD),
            self.center[1] + self.inner_radius * math.sin(self.angle * DEG2RAD),
        )
        ec = (
            self.center[0] + self.outer_radius * math.cos(self.angle * DEG2RAD),
            self.center[1] + self.outer_radius * math.sin(self.angle * DEG2RAD),
        )
        pts += [
            (
...
class ShapeParabolic(Shape):
    ...
    def define_points(self, pts):
        ...
        while not READY:
            ...
            east_shape.append(
                (coord[0] + sinangle * width / 2.0, coord[1] - cosangle * width / 2.0)
            )
            west_shape.append(
                (coord[0] - sinangle * width / 2.0, coord[1] + cosangle * width / 2.0)
            )
