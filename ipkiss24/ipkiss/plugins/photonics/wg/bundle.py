...
__all__ = ["WgElBundleWaveguides", "WgElBundle", "WgElBundleConnectRounded"]
...
class WgElBundleWaveguides(Group):
    ...
    waveguides = RestrictedProperty(
        restriction=RestrictTypeList(Group),
        required=True)  # maybe waveguide later on

    def define_waveguides(self):
        raise NotImplementedException(
            "Subclass of WgElBundleWaveguides should implement method define_waveguides."
        )
    ...
    def define_elements(self, elems):
        ...
        for (wg1, Wg2) in zip(self.waveguides[:-1], self.waveguides[1:]):
            ...
            if D0 > D_ref:
                ST_1 += S1[0].move_polar_copy(0.5 * w1 + t1,
                                              A1[0] + orientation * 90.0)
                ST_1 += S2[0].move_polar_copy(0.5 * w2 + t2,
                                              A2[0] - orientation * 90.0)
            ST_1 += S2[0].move_polar_copy(0.5 * w2, A2[0] - orientation * 90.0)

            ST_2 += S1[-1].move_polar_copy(0.5 * w1,
                                           A1[-2] + orientation * 90.0)
            if D1 > D_ref:
                ST_2 += S1[-1].move_polar_copy(0.5 * w1 + t1,
                                               A1[-2] + orientation * 90.0)
                ST_2 += S2[-1].move_polar_copy(0.5 * w2 + t2,
                                               A2[-2] - orientation * 90.0)
            ST_2 += S2[-1].move_polar_copy(0.5 * w2,
                                           A2[-2] - orientation * 90.0)
...
class WgElBundleGeneric(WgElBundleWaveguides):
    ...
    wg_widths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), required=True)
    trench_widths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), required=True)

    def __make_wg_element__(shape, wg_width, trench_width, process):
        wg_def = WgElDefinition(
            wg_width=wg_width, trench_width=trench_width, process=process)
        return wg_def(shape=shape)

    def define_waveguides(self):
        if (len(self.shapes) != len(self.wg_widths)
                or len(self.wg_widths) != len(self.trench_widths)):
            raise AttributeError(
                "Length of shapes, wg_widths and trench_widths should be identical in WgElBundleGeneric"
            )
        waveguides = [
            self.__make_wg_element__(S, w, t, self.process)
            for (S, w,
                 t) in zip(self.shapes, self.wg_widths, self.trench_widths)
        ]
        return waveguides
...
class WgElBundleConnectRoundedGeneric(__RoundedWaveguide__, WgElBundleGeneric):
    ...
    bend_radii = RestrictedProperty(
        required=True, restriction=RestrictList(RESTRICT_POSITIVE))

    def define_waveguides(self):
        waveguides = []
        for (S, w, t, r) in zip(self.shapes, self.wg_widths,
                                self.trench_widths, self.bend_radii):
            wg_def = WgElDefinition(
                wg_width=w, trench_width=t, process=self.process)
...
class WgElBundleConnectRounded(__IdenticalWaveguides__,
                               WgElBundleConnectRoundedGeneric):
