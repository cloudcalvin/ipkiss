...
class __ProcessSuperpositionMaterialStackGeometry2DPolygons__(
        CartesianGeometry2D):
    ...
    processes = ListProperty(
        required=True,
        doc=
        "list of all the processes (in the same order as the columns are listed in the mapping table)"
    )
    mapping_definition = ListProperty(
        required=True,
        doc=
        "list with tuples (processes_present, processes_not_present, material_stack) / material_stack"
    )
    process_polygons = DictProperty(
        required=True,
        doc=
        "dictionary : key : process / value: object of type LayerShapelyPolygons"
    )
    material_stacks_shapely_polygons = DefinitionProperty(
        doc=
        "contains the resulting geometry represented as tuple (material_stack_id, shapely_polygon)"
    )
    material_stacks_shapes = DefinitionProperty(
        doc=
        "contains the resulting geometry represented as tuple (material_stack_id, shape)"
    )
    ...
    def __get_mapping_internal_format__(self):
        ...
        for pflags, mstack in self.mapping_definition:
            ...
            process_to_material_stack_map.append((
                tuple(true_list),
                tuple(false_list),
                mstack,
            ))
    ...
    def define_canvas_size(self):
        canvas_size = (int(numpy.ceil(self.size_info.width / self.grid)), int(
            numpy.ceil(self.size_info.height / self.grid)))
        return canvas_size
    ...
    def define_material_stacks_shapely_polygons(self):
        ...
        for (process_set, exclude_process_set,
             material_stack) in self.__get_mapping_internal_format__():
            current_len = len(process_set)
            if (current_len == 0):
                empty_polygon = LayerShapelyPolygons(
                    layer=Layer(
                        number=0, name="VFABRICATION"
                    ),  # FIXME: will result in problem if this layer is in use
                    size_info=self.size_info)
                canvas_polygon = empty_polygon.bitwise_not()
                id = self.material_stack_factory.get_material_stack_id(
                    material_stack)
                ...
                for p in process_set:
                    if (process_polygon is None):
                        ...
                        process_polygon = process_polygon.bitwise_and(
                            self.process_polygons[p])
                    if (self.save_debug_images):
                        process_polygon.save_to_image(
                            "DEBUG_PROCESS_POLYGONS__%i__%i__AND_%s.png" %
                            (debug_counter_line, debug_counter, p.extension))
                    debug_counter = debug_counter + 1
                for e_p in exclude_process_set:
                    process_polygon = process_polygon.difference(
                        self.process_polygons[e_p])
                    if (self.save_debug_images):
                        process_polygon.save_to_image(
                            "DEBUG_PROCESS_POLYGONS__%i__%i__DIFFERENCE_%s.png"
                            % (debug_counter_line, debug_counter,
                               e_p.extension))
                    debug_counter = debug_counter + 1
                process_polygon.unionize()
                if (self.save_debug_images):
                    process_polygon.save_to_image(
                        "DEBUG_PROCESS_POLYGONS__%i__%i__UNION.png" %
                        (debug_counter_line, debug_counter))
                id = self.material_stack_factory.get_material_stack_id(
                    material_stack)
    ...
    def define_scaler(self):
        ...
        ]  #FIXME - Scaler klasse hanteert een andere semantiek voor box... :(
    ...
    def __debug_savefig_material_array__(self, material_array, count,
                                         count_progress, material_stack_id):
        if self.save_debug_images:
            #-debug code-
            ...
            pyplot.savefig(
                str(count) + "_" +
                self.material_stack_factory[material_stack_id].name.replace(
                    " ",
                    "_") + "_" + str(count_progress) + "_MATERIAL_ARRAY.png")
            #-end of debug code-

    def __debug_savefig_polygon__(self, polygon_points, count, count_progress,
                                  material_stack_id):
        if self.save_debug_images:
            #-debug code-
            from .vfabrication import LayerShapelyPolygons
            debug_shape = Shape(polygon_points)
            debug_lbm = LayerShapelyPolygons(
                layer=Layer(number=0, name="VFABRICATION_EMPTY"),
                size_info=SizeInfo(
                    west=0,
                    east=self.canvas_size[0],
                    south=0,
                    north=self.canvas_size[1]))
            debug_lbm.add_shape(debug_shape)
            debug_lbm.save_to_image(
                str(count) + "_" +
                self.material_stack_factory[material_stack_id].name.replace(
                    " ", "_") + "_" + str(count_progress) + ".png")
            #-end of debug code-
    ...
    def get_material_array(self):
        ...
        if (self.__material_array__ is None):
            ...
            mat_bm[0] = (
                mat_bm[0][0], None
            )  #replace the canvas polygon (1st element) by "None" : this is more efficient and will be interpreted identically later on
            material_array = numpy.zeros(
                [self.canvas_width, self.canvas_height],
                dtype=numpy.int16)  #store the material id as 16-bit integer
            count = 0
            for (material_stack_id, mb) in mat_bm:
                LOG.debug(
                    "Geometry : calculating material array for material_stack_id %s..."
                    % self.material_stack_factory[material_stack_id])
                if (mb is None):
                    if (count != 0):
                        raise Exception(
                            "Unexpected error : the first element of the 'material_stacks_shapely_polygons' list is supposed to contain the background material."
                        )
                    pass
                    ...
                    if not mb.georep.is_empty:
                        ...
                        for polygon in mb.georep_list:
                            if polygon.is_ring:
                                ...
                                self.__debug_savefig_polygon__(
                                    polygon_points, count, count_progress,
                                    material_stack_id)
                                polygon_points = self.__scale_polygon_points(
                                    polygon_points)
                                __common_function_apply_polygon_to_array_memory_sparing__(
                                    material_array,
                                    polygon_points,
                                    do_bitwise_or=False,
                                    value=material_stack_id)
                                self.__debug_savefig_material_array__(
                                    material_array, count, count_progress,
                                    material_stack_id)
                                progress_percent = count_progress / total_geoms * 100.0
                                LOG.debug(
                                    "%i percent done..." % progress_percent)
                            else:
                                #outer polygon
                                outer_polygon_points = polygon.exterior.coords
                                outer_polygon_points = self.__scale_polygon_points(
                                    outer_polygon_points)
                                #FIXME - array can be made smaller to save memory (just covering the polygon)
                                mat_array = material_array_outer_polygon = numpy.zeros(
                                    [self.canvas_width, self.canvas_height],
                                    dtype=numpy.int16)
                                __common_function_apply_polygon_to_array_memory_sparing__(
                                    material_array_outer_polygon,
                                    outer_polygon_points,
                                    do_bitwise_or=False,
                                    value=material_stack_id)
                                #inner polygons
                                for ip in polygon.interiors:
                                    ...
                                    inner_polygon_points = self.__scale_polygon_points(
                                        inner_polygon_points)
                                    material_array_inner_polygon = numpy.zeros(
                                        [
                                            self.canvas_width,
                                            self.canvas_height
                                        ],
...
class ProcessSuperpositionVirtualFabrication2DGeometryPolygons(
        __ProcessSuperpositionMaterialStackGeometry2DPolygons__,
        __UniformEnvironmentGeometry__):
    pass


class ProcessSuperpositionVirtualFabrication3DGeometryPolygons(
        ProcessSuperpositionVirtualFabrication2DGeometryPolygons):
    size_z = FunctionNameProperty(fget_name="get_size_z")

    def get_size_z(self):
        mat_bm = self.material_stacks_shapely_polygons
        material_stack_id = mat_bm[1][
            0]  #take the second element of mat_bm, not the first as that is the canvas polygon
