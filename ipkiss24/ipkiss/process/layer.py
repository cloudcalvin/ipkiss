    ...
    def __call__(cls, *params, **keyword_params):
        ...
        if 'extension' in keyword_params:
            ...
            raise AttributeError(
                "Extension for a process layer should not be empty. Reset to XX"
            )
...
class MetaPatternPurposeCreator(MetaPropertyInitializer):
    ...
    def __call__(cls, *params, **keyword_params):
        ...
        if 'extension' in keyword_params:
            ...
            extension = "XX"
            raise AttributeError(
                "Extension for a pattern purpose should not be empty. Reset to XX"
            )
...
class ProcessLayerList(list):
    ...
    def __delitem__(self, key):
        if isinstance(key, str):
            for i in range(0, len(self)):
                if list.__getitem__(self, i).extension == key:
