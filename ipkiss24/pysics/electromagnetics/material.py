...
class __BlendedElectroMagneticMaterial__(__ElectroMagneticMaterial__):
    ...
    def get_epsilon(self, environment=DEFAULT_ENVIRONMENT):
        return self.fraction * self.material_1.get_epsilon(environment) + (
            1 - self.fraction) * self.material_2.get_epsilon(environment)

    def get_mu(self):
        return self.fraction * self.material_1.get_mu(environment) + (
            1 - self.fraction) * self.material_2.get_mu(environment)
...
if hasattr(TECH, "MATERIALS"):
    for (material_id, material) in TECH.MATERIALS:
        if hasattr(material, "epsilon"):
            raise Exception(
                "Epsilon value of material '%s' is about to be overwritten by mixin of class __ElectroMagneticMaterial__.\nMake sure you do \"from pysics.electromagnetics import *\" in your technology settings before setting epsilon values."
                % material.name)
