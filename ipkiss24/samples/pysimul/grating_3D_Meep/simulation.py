...
files_to_copy = [
    'simulation.py', 'ComponentSimulator3D.py', 'Filters_OL_new.py',
    'layout.py', 'Technology.py'
]
...
reference_component_simulator = StructureMeep3DSimulator(
    ...
    growth=0.0,
    wavelength=1.55,
    source_input_port_offset=-section_len /
    2.0)  #offset for source in reference to the input port
...
reference_component_simulator.add_output_cut(
    OutputCut(normal_vector='y', filename='%s/Ey_y_ref' %
              output_directory))  #slice orthogonal to the Y-axis at y=0
reference_component_simulator.add_output_cut(
    OutputCut(
        normal_vector='z',
        cut_value=0.810,
        filename='%s/Ey_z1_ref' % output_directory)
...
reference_component_simulator.add_output_cut(
    OutputCut(
        normal_vector='z',
        cut_value=0.81,
        filename='%s/Ey_z1' % output_directory))
