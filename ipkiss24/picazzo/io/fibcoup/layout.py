...
__all__ = ["IoFibcoupGeneric", "IoFibcoupAsymmetric", "IoFibcoup"]
...
class IoFibcoupGeneric(IoBlockAdapter):
    ...
    west_taper_lengths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[300.0])
    west_wg_widths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[3.0])
    west_trench_widths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE),
        default=[TECH.WG.TRENCH_WIDTH])
    west_connect_lengths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[40.0])
    west_bend_radiuses = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE),
        default=[TECH.WG.BEND_RADIUS])
    west_minimum_straights = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE),
        default=[TECH.WG.SHORT_STRAIGHT])
    west_fibcoups = RestrictedProperty(
        restriction=RestrictTypeList(Structure),
        default=[TECH.IO.FIBCOUP.DEFAULT_GRATING])
    west_fibcoup_offsets = RestrictedProperty(
        restriction=RestrictList(RESTRICT_NUMBER), default=[25.6])
    west_fibcoup_taper_lengths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[350.0])
    west_fibcoups_transforms_and_positions = DefinitionProperty()
    west_merged_waveguides = BoolProperty(default=True)

    east_taper_lengths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[300.0])
    east_wg_widths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[3.0])
    east_trench_widths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE),
        default=[TECH.WG.TRENCH_WIDTH])
    east_connect_lengths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[40.0])
    east_bend_radiuses = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE),
        default=[TECH.WG.BEND_RADIUS])
    east_minimum_straights = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE),
        default=[TECH.WG.SHORT_STRAIGHT])
    east_fibcoups = RestrictedProperty(
        restriction=RestrictTypeList(Structure),
        default=[TECH.IO.FIBCOUP.DEFAULT_GRATING])
    east_fibcoup_offsets = RestrictedProperty(
        restriction=RestrictList(RESTRICT_NUMBER), default=[25.6])
    east_fibcoup_taper_lengths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[350.0])
    ...
    def define_name(self):
        return "%s_FC%d_S%s_H%d_%d_%d" % (
            self.__name_prefix__, do_hash("".join(
                [S.name for S in self.west_fibcoups + self.east_fibcoups])),
            self.struct.name, do_hash(
                str(self.offset) + str(self.y_spacing) + str(self.south_west) +
                str(self.south_east)), do_hash(self.west_merged_waveguides) +
            do_hash(self.east_merged_waveguides), do_hash(
                sum(self.west_fibcoup_offsets) + sum(self.east_fibcoup_offsets)
                + sum(self.west_bend_radiuses) + sum(self.east_bend_radiuses) +
                sum(self.west_connect_lengths) + sum(self.east_connect_lengths)
                + sum(self.west_minimum_straights) + sum(
                    self.east_minimum_straights) + sum(
                        self.west_fibcoup_taper_lengths) +
                sum(self.east_fibcoup_taper_lengths) + sum(
                    self.west_taper_lengths) + sum(self.east_taper_lengths) +
                sum(self.west_trench_widths) + sum(self.east_trench_widths) +
    ...
    def define_west_fibcoups_transforms_and_positions(self):
        ...
        for ypos in self.y_west:
            ...
            west_fibcoup_position = (
                self.west_fibcoup_offsets[i % len(self.west_fibcoup_offsets)],
                ypos - fc.east_ports[0].y)
            west_fibcoup_and_position.append((fc, west_fibcoup_position,
                                              IdentityTransform()))
            i += 1
    ...
    def define_east_fibcoups_transforms_and_positions(self):
        ...
        for ypos in self.y_east:
            ...
            east_fibcoup_position = (
                self.width -
                self.east_fibcoup_offsets[i % len(self.east_fibcoup_offsets)],
                ypos - fc.east_ports[0].y)
            east_fibcoup_and_position.append((fc, east_fibcoup_position,
                                              HMirror()))
            i += 1
    ...
    def define_elements(self, elems):
        ...
        for i in range(len(self.struct_west_ports)):
            ...
            west_fibcoup_length = west_fibcoup_port.position[0] + self.west_fibcoup_offsets[i % len(
                self.west_fibcoup_offsets)]
            ...
            start_wg_def1 = WgElDefinition(
                ...
                trench_width=ip.wg_definition.trench_width,
                process=west_process)
            end_wg_def1 = WgElDefinition(
                wg_width=west_wg_width,
            ...
            start_wg_def2 = WgElDefinition(
                wg_width=west_wg_width,
                trench_width=west_trench_width,
                process=west_process)
            end_wg_def2 = WgElDefinition(
                wg_width=west_fibcoup_width,
        ...
        if len(self.struct_west_ports):
            if self.west_merged_waveguides:
                ...
                for (s, w, t, r, p) in zip(shapes, wg_widths, trench_widths,
                                           radiuses, processes):
                    wg_def = WgElDefinition(
                        wg_width=w, trench_width=t, process=p)
        ...
        for i in range(len(self.struct_east_ports)):
            ...
            east_fibcoup_length = east_fibcoup_port.position[0] + self.east_fibcoup_offsets[i % len(
                self.east_fibcoup_offsets)]
            ...
            start_wg_def3 = WgElDefinition(
                ...
                trench_width=op.wg_definition.trench_width,
                process=east_process)
            end_wg_def3 = WgElDefinition(
                wg_width=east_wg_width,
            ...
            # draw fibcoup taper
            start_wg_def4 = WgElDefinition(
                wg_width=east_wg_width,
                trench_width=east_trench_width,
                process=east_process)
            end_wg_def4 = WgElDefinition(
                wg_width=east_fibcoup_width,
    ...
    def define_ports(self, ports):
        ...
        for p in self.struct.ports:
            if isinstance(p, VerticalOpticalPort):
                vop = p.transform_copy(
                    self.struct_transformation).move_copy(struct_position)
...
class IoFibcoupAsymmetric(IoFibcoupGeneric):
    ...
    west_minimum_straight = PositiveNumberProperty(
        default=TECH.WG.SHORT_STRAIGHT)
    ...
    east_minimum_straight = PositiveNumberProperty(
        default=TECH.WG.SHORT_STRAIGHT)
    ...
    west_taper_lengths = DefinitionProperty(
        fdef_name="define_west_taper_lengths")
    west_wg_widths = DefinitionProperty(fdef_name="define_west_wg_widths")
    west_trench_widths = DefinitionProperty(
        fdef_name="define_west_trench_widths")
    west_connect_lengths = DefinitionProperty(
        fdef_name="define_west_connect_lengths")
    west_bend_radiuses = DefinitionProperty(
        fdef_name="define_west_bend_radiuses")
    west_minimum_straights = DefinitionProperty(
        fdef_name="define_west_minimum_straights")
    west_fibcoups = DefinitionProperty(fdef_name="define_west_fibcoups")
    west_fibcoup_offsets = DefinitionProperty(
        fdef_name="define_west_fibcoup_offsets")
    west_fibcoup_taper_lengths = DefinitionProperty(
        fdef_name="define_west_fibcoup_taper_lengths")

    east_taper_lengths = DefinitionProperty(
        fdef_name="define_east_taper_lengths")
    east_wg_widths = DefinitionProperty(fdef_name="define_east_wg_widths")
    east_trench_widths = DefinitionProperty(
        fdef_name="define_east_trench_widths")
    east_connect_lengths = DefinitionProperty(
        fdef_name="define_east_connect_lengths")
    east_bend_radiuses = DefinitionProperty(
        fdef_name="define_east_bend_radiuses")
    east_minimum_straights = DefinitionProperty(
        fdef_name="define_east_minimum_straights")
    east_fibcoups = DefinitionProperty(fdef_name="define_east_fibcoups")
    east_fibcoup_offsets = DefinitionProperty(
        fdef_name="define_east_fibcoup_offsets")
    east_fibcoup_taper_lengths = DefinitionProperty(
        fdef_name="define_east_fibcoup_taper_lengths")

    def define_name(self):
        return "%s_%d_%d_%s_%d_H%d" % (
            self.__name_prefix__, do_hash(self.west_fibcoup),
            do_hash(self.east_fibcoup), self.struct.name, do_hash(
                str((str(self.offset) + str(self.y_spacing) +
                     str(self.south_west) + str(self.south_east)))),
            self.west_bend_radius + self.east_bend_radius +
            self.west_connect_length + self.east_connect_length +
            self.west_fibcoup_offset + self.east_fibcoup_offset +
            self.west_fibcoup_taper_length + self.east_fibcoup_taper_length +
            self.west_minimum_straight + self.east_minimum_straight + self.
            west_taper_length + self.east_taper_length + self.west_trench_width
            + self.east_trench_width + self.west_wg_width + self.east_wg_width)
...
class IoFibcoup(IoFibcoupAsymmetric):
    ...
    west_taper_length = DefinitionProperty(
        fdef_name="define_west_taper_length")
    west_wg_width = DefinitionProperty(fdef_name="define_west_wg_width")
    west_trench_width = DefinitionProperty(
        fdef_name="define_west_trench_width")
    west_connect_length = DefinitionProperty(
        fdef_name="define_west_connect_length")
    west_bend_radius = DefinitionProperty(fdef_name="define_west_bend_radius")
    west_minimum_straight = DefinitionProperty(
        fdef_name="define_west_minimum_straight")
    west_fibcoup = DefinitionProperty(fdef_name="define_west_fibcoup")
    west_fibcoup_offset = DefinitionProperty(
        fdef_name="define_west_fibcoup_offset")
    west_fibcoup_taper_length = DefinitionProperty(
        fdef_name="define_west_fibcoup_taper_length")
    west_merged_waveguides = DefinitionProperty(
        fdef_name="define_west_merged_waveguides")

    east_taper_length = DefinitionProperty(
        fdef_name="define_east_taper_length")
    east_wg_width = DefinitionProperty(fdef_name="define_east_wg_width")
    east_trench_width = DefinitionProperty(
        fdef_name="define_east_trench_width")
    east_connect_length = DefinitionProperty(
        fdef_name="define_east_connect_length")
    east_bend_radius = DefinitionProperty(fdef_name="define_east_bend_radius")
    east_minimum_straight = DefinitionProperty(
        fdef_name="define_east_minimum_straight")
    east_fibcoup = DefinitionProperty(fdef_name="define_east_fibcoup")
    east_fibcoup_offset = DefinitionProperty(
        fdef_name="define_east_fibcoup_offset")
    east_fibcoup_taper_length = DefinitionProperty(
        fdef_name="define_east_fibcoup_taper_length")
    east_merged_waveguides = DefinitionProperty(
        fdef_name="define_east_merged_waveguides")

    def define_name(self):
        return "%s_%s_%s_%d_H%d" % (
            self.__name_prefix__, self.fibcoup.name, self.struct.name, do_hash(
                str((str(self.offset) + str(self.y_spacing) +
                     str(self.south_west) + str(self.south_east)))),
            self.bend_radius + self.connect_length + self.fibcoup_offset +
            self.fibcoup_taper_length + self.minimum_straight +
            self.taper_length + self.trench_width + self.wg_width)
