...
from ipkiss.plugins.photonics.port.port_list import (
    OpticalPortList,
...
class IoFibcoupGeneric(IoBlockAdapter):
    ...
    west_trench_widths = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[TECH.WG.TRENCH_WIDTH]
    )
    ...
    west_bend_radiuses = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[TECH.WG.BEND_RADIUS]
    )
    west_minimum_straights = RestrictedProperty(
        restriction=RestrictList(RESTRICT_POSITIVE), default=[TECH.WG.SHORT_STRAIGHT]
    )
    ...
    def define_name(self):
        return "%s_FC%d_S%s_H%d_%d_%d" % (
            ...
            do_hash(
                sum(self.west_fibcoup_offsets)
                + sum(self.east_fibcoup_offsets)
                + sum(self.west_bend_radiuses)
                + sum(self.east_bend_radiuses)
                + sum(self.west_connect_lengths)
                + sum(self.east_connect_lengths)
                + sum(self.west_minimum_straights)
                + sum(self.east_minimum_straights)
                + sum(self.west_fibcoup_taper_lengths)
                + sum(self.east_fibcoup_taper_lengths)
                + sum(self.west_taper_lengths)
                + sum(self.east_taper_lengths)
                + sum(self.west_trench_widths)
                + sum(self.east_trench_widths)
                + sum(self.west_wg_widths)
                + sum(self.east_wg_widths)
            ),
        )
    ...
    def define_west_fibcoups_transforms_and_positions(self):
        ...
        for ypos in self.y_west:
            ...
            west_fibcoup_and_position.append(
                (fc, west_fibcoup_position, IdentityTransform())
            )
            i += 1
    ...
    def define_elements(self, elems):
        ...
        for i in range(len(self.struct_west_ports)):
            ...
            west_connect_length = self.west_connect_lengths[
                i % len(self.west_connect_lengths)
            ]
            west_taper_length = self.west_taper_lengths[
                i % len(self.west_taper_lengths)
            ]
            west_trench_width = self.west_trench_widths[
                i % len(self.west_trench_widths)
            ]
            ...
            west_minimum_straight = self.west_minimum_straights[
                i % len(self.west_minimum_straights)
            ]
            ...
            t_pos = (
                west - west_connect_length - TECH.WG.SHORT_STRAIGHT,
                self.y_west[i],
            )
            ...
            elems += WgElTaperLinear(
                start_position=(
                    west_fibcoup_length + west_fibcoup_taper_length,
                    t_pos[1],
                ),
        ...
        if len(self.struct_west_ports):
            if self.west_merged_waveguides:
                ...
                for (s, w, t, r, p) in zip(
                    shapes, wg_widths, trench_widths, radiuses, processes
                ):
        ...
        for i in range(len(self.struct_east_ports)):
            ...
            east_connect_length = self.east_connect_lengths[
                i % len(self.east_connect_lengths)
            ]
            east_taper_length = self.east_taper_lengths[
                i % len(self.east_taper_lengths)
            ]
            east_trench_width = self.east_trench_widths[
                i % len(self.east_trench_widths)
            ]
            ...
            east_minimum_straight = self.east_minimum_straights[
                i % len(self.east_minimum_straights)
            ]
            ...
            # position taper
            t_pos = (
                east + east_connect_length + TECH.WG.SHORT_STRAIGHT,
                self.y_east[i],
            )
            wg_def = WgElDefinition(
    ...
    def define_ports(self, ports):
        ...
        for p in self.struct.ports:
            if isinstance(p, VerticalOpticalPort):
                vop = p.transform_copy(self.struct_transformation).move_copy(
                    struct_position
                )
...
class IoFibcoupAsymmetric(IoFibcoupGeneric):
    ...
    def define_name(self):
        return "%s_%d_%d_%s_%d_H%d" % (
            self.__name_prefix__,
            do_hash(self.west_fibcoup),
            do_hash(self.east_fibcoup),
            self.struct.name,
            do_hash(
                str(
                    (
            ...
            self.west_bend_radius
            + self.east_bend_radius
            + self.west_connect_length
            + self.east_connect_length
            + self.west_fibcoup_offset
            + self.east_fibcoup_offset
            + self.west_fibcoup_taper_length
            + self.east_fibcoup_taper_length
            + self.west_minimum_straight
            + self.east_minimum_straight
            + self.west_taper_length
            + self.east_taper_length
            + self.west_trench_width
            + self.east_trench_width
            + self.west_wg_width
            + self.east_wg_width,
        )
...
class IoFibcoup(IoFibcoupAsymmetric):
    ...
    def define_name(self):
        return "%s_%s_%s_%d_H%d" % (
            ...
            + self.connect_length
            + self.fibcoup_offset
            + self.fibcoup_taper_length
            + self.minimum_straight
            + self.taper_length
            + self.trench_width
            + self.wg_width,
        )
