...
__all__ = [
    "GenericW1Waveguide", "W1Waveguide", "W1WaveguideWithInlineCavity",
    "W1WaveguideWithAllpass"
]


class __GenericW1Slice__(__PhCAutoMap__, TriangularPhCLayout):
    unit_cells = RestrictedProperty(
        required=True, doc="List of unit cells: center to outside")
...
class GenericW1WaveguideSection(__GenericW1Waveguide__):
    ...
    def define_name(self):
        ...
        port_width = sqrt(3.0) * self.lattice_pitches[
            1] if self.port_width is None else self.port_width
        name = ("%s_P%d_%d_U%d_L%d_P%d_%d_%d_E%d_%s_%s" %
                (self.__name_prefix__, self.lattice_pitches[0] * 1000,
                 self.lattice_pitches[1] * 1000,
                 do_hash("".join([u.name for u in self.unit_cells])),
                 self.n_o_periods, port_row, port_width * 1000,
                 self.port_offset * 1000, int(self.start_even),
    ...
    def define_ports_coordinates(self):
        ...
        wg_def = WgElDefinition(
            wg_width=sqrt(3.0) * self.lattice_pitches[1]
            if self.port_width is None else self.port_width,
            trench_width=TECH.WG.TRENCH_WIDTH,
            process=TECH.PROCESS.WG)
        port_row = (len(self.unit_cells) - 1
                    ) / 2 if self.port_row is None else self.port_row
...
class GenericW1WaveguideTermination(__GenericW1Slice__):
    ...
    def define_name(self):
        ...
        name = ("%s_P%d_%d_U%d_P%d_%d_%d_E%d_%s_%s" %
                (self.__name_prefix__, self.lattice_pitches[0] * 1000,
                 self.lattice_pitches[1] * 1000,
                 do_hash("".join([u.name for u in self.unit_cells])), port_row,
                 port_width * 1000, self.port_offset * 1000,
                 int(self.start_even), self.process_wg.extension,
    ...
    def define_ports_coordinates(self):
        ...
        return [((-port_col, -port_row), -180, wg_def), ((port_col, -port_row),
                                                         0.0, wg_def)]
...
class GenericW1Waveguide(__GenericW1Waveguide__):
    ...
    def __init__(self,
                 ...
                 unit_cells,
                 n_o_periods,
                 process_wg=TECH.PROCESS.WG,
                 process_hfw=TECH.PROCESS.HFW,
                 **kwargs):
...
class __W1__(DodecPhCLayout):
    ...
    def define_ports_coordinates(self):
        ...
        port_row = 0.0
        port_col = (
            0.5 * cos(pi / 12.0) * self.diameter + TECH.TECH.MINIMUM_LINE
        ) / self.pitch
...
class W1WaveguideWithInlineCavity(__W1__):
    ...
    def define_name(self):
        ...
        return "%s_A%d_D%d_C%d_L%d_DC%d_CH%s_%s_%s" % (
            self.__name_prefix__, self.pitch * 1000, self.diameter * 1000,
            self.n_o_cladding_layers, self.n_o_periods,
            self.defect_diameter * 1000, cavity_holes_name,
            self.process_wg.extension, self.process_hfw.extension)
    ...
    def define_hole_sizes_map(self):
        ...
        for i in range(self.n_o_cladding_layers, 0, -1):
            if i % 2:
                ...
                maplist.append(" /" + str(self.n_o_periods - 1) + ",A /")
        n_core = int(
            floor((self.n_o_periods - 2 * len(self.cavity_holes) - 1) / 2.0))
        maplist.append("/" + str(n_core) + ", Z/ " + cav_str + "/" +
                       str(n_core) + ",Z /")
...
class W1WaveguideWithAllpass(__W1__):
    ...
    def define_hole_sizes_map(self):
        ...
        maplist.append(
            "/" + str(int(ceil(self.n_o_periods / 2 - 1))) + ",A /Z /" +
            str(self.n_o_periods - int(ceil(self.n_o_periods / 2))) + ",A /")
        maplist.append(" /" + str(self.n_o_periods - 1) + ",A /")
        maplist.append("/" + str(self.n_o_periods) + ",A /")
