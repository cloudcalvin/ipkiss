...
class CoupledRingGeneric(Structure):
    ...
    first_ring = RestrictedProperty(
        restriction=RestrictType(__Ring__), required=True)
    middle_ring = RestrictedProperty(
        restriction=RestrictType(__Ring__), required=True)
    last_ring = RestrictedProperty(
        restriction=RestrictType(__Ring__), required=True)
    spacings = RestrictedProperty(
    ...
    def __get_MR__(self):
        ...
        return MultiRingIdenticalWithChangedInAndOutWithSpacings(
            self.middle_ring, self.first_ring, self.last_ring, self.spacings,
            True)

    def define_elements(self, elems):
        elems += SRef(self.__get_MR__(), (0.0, 0.0),
...
class CoupledRingRectSymm(CoupledRingGeneric):
    ...
    def __init__(self, spacings=[TECH.WG.SPACING], **kwargs):
        super(CoupledRingRectSymm, self).__init__(
            spacings=spacings[1:-1], **kwargs)
        self._first_spacing = spacings[0]
        self._last_spacing = spacings[-1]

    def define_first_ring(self):
        return RingRectSymmNotchFilter(
            coupler_wg_definitions=[self.wg_definition],
