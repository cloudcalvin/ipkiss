...
__all__ = [
    "MultiRing", "MultiRingIdentical", "MultiRingIdenticalWithChangedInAndOut",
    "MultiRingIdenticalWithChangedInAndOutWithSpacing",
    "MultiRingIdenticalWithChangedInAndOutWithSpacings",
    "MultiRingIdenticalWithSpacing", "MultiRingIdenticalWithSpacings",
    "MultiRingPeriodic", "MultiRingWithGaps"
]
...
class MultiRing(Structure):
    ...
    rings = RestrictedProperty(
        restriction=RestrictTypeList(__Ring__), required=True)
    ring_transformations = RestrictedProperty(
        restriction=RestrictTypeList(NoDistortTransform), required=True)

    def define_elements(self, elems):
        if len(self.rings) != len(self.ring_transformations):
            raise IpcoreAttributeException(
                "length of ring list is different from length of ring_transformations list"
            )
...
class MultiRingIdentical(MultiRing):
    ...
    ring = RestrictedProperty(
        restriction=RestrictType(__Ring__), required=True)
...
class MultiRingPeriodic(MultiRingIdentical):
    ...
    ring_transformations = DefinitionProperty(
        fdef_name="define_ring_transformations")
    ...
    def define_ring_transformations(self):
        return [
            Translation((self.origin[0] + i * self.period[0],
                         self.origin[1] + i * self.period[1]))
            for i in range(self.n_o_rings)
        ]


def MultiRingIdenticalWithChangedInAndOut(ring_middle, ring_in, ring_out,
                                          ring_transformations, **kwargs):
    ...
    rings = [ring_in
                  ...
                  for i in range(len(ring_transformations) - 2)] + [ring_out]
    return MultiRing(
        rings=rings, ring_transformations=ring_transformations, **kwargs)
...
def MultiRingWithSpacings(rings, spacings, flip_last=False, **kwargs):
    ...
    for i in range(len(spacings) - 1):
        ring_transformations += [
            ring_transformations[-1] + Translation(
                (0.0, 0.5 * ss[i][1] + 0.5 * ss[i + 1][1] + bs[1] + bs[i + 1] +
                 spacings[i]))
        ]
    if len(rings) > 1:
        if flip_last:
            ring_transformations.append(
                CMirror() + ring_transformations[-1] + Translation(
                    (0.0, 0.5 * ss[-1][1] + 0.5 * ss[-2][1] + bs[-1] + bs[-2] +
                     spacings[-1])))
        else:
            ring_transformations.append(ring_transformations[-1] + Translation(
                (0.0, 0.5 * ss[-1][1] + 0.5 * ss[-2][1] + bs[-1] + bs[-2] +
                 spacings[-1])))
    return MultiRing(
        rings=rings, ring_transformations=ring_transformations, **kwargs)
...
class MultiRingIdenticalWithSpacings(MultiRingIdentical):
    spacings = ListDefinitionProperty(
        allowed_types=[int, float], restriction=RESTRICT_POSITIVE)
    ring_transformations = DefinitionProperty(
        fdef_name="define_ring_transformations")
    ...
    def define_ring_transformations(self):
        ...
        for s in self.spacings:
            ring_transformations.append(ring_transformations[-1] + Translation(
                (0.0, self.ring.straights[1] +
                 2 * numpy.mean(self.ring.get_bend90_size()) + s)))
...
def MultiRingIdenticalWithChangedInAndOutWithSpacings(ring_middle,
                                                      ...
                                                      spacings,
                                                      flip_last=False):
    rings = [ring_in] + [ring_middle
                         for i in range(len(spacings) - 1)] + [ring_out]
    return MultiRingWithSpacings(
        rings=rings, spacings=spacings, flip_last=flip_last)
...
def MultiRingIdenticalWithChangedInAndOutWithSpacing(ring_middle,
                                                     ...
                                                     flip_last=False):
    spacings = [spacing for i in range(n_o_rings - 1)]
    return MultiRingIdenticalWithChangedInAndOutWithSpacings(
