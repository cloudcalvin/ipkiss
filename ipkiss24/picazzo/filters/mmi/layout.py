...
__all__ = [
    "MmiUniformWgWidth", "Mmi", "MmiTapered", "Mmi1x2", "Mmi1x2Tapered",
    "Mmi2x1Tapered", "Mmi2x2", "Mmi2x2Tapered", "MmiTaperPorts",
    "__MmiTaperPorts__", "__MmiSymmetric__"
]
...
class MmiBasic(Structure):
    ...
    mmi_wg_definition = WaveguideDefProperty(
        required=True, doc="waveguide definition used to define the MMI core")
    input_wg_definitions = WaveguideDefListProperty(
        default=[], doc="waveguide definitions of the inputs")
    output_wg_definitions = WaveguideDefListProperty(
        default=[], doc="waveguide definitions of the outputs")
    input_y_positions = RestrictedProperty(
        restriction=RestrictList(RESTRICT_NUMBER), default=[])
    output_y_positions = RestrictedProperty(
        restriction=RestrictList(RESTRICT_NUMBER), default=[])
    ...
    def define_ports(self, ports):
        for wd, y in zip(self.input_wg_definitions, self.input_y_positions):
            ports += InOpticalPort(
                wg_definition=wd, position=(0.0, y), angle=180.0)
        for wd, y in zip(self.output_wg_definitions, self.output_y_positions):
            ports += OutOpticalPort(
                wg_definition=wd, position=(self.length, y), angle=0.0)
        return ports
...
class MmiUniformWgWidth(MmiBasic):
    ...
    input_wg_definitions = DefinitionProperty(
        fdef_name="define_input_wg_definitions")
    output_wg_definitions = DefinitionProperty(
        fdef_name="define_output_wg_definitions")
    wg_definition = WaveguideDefProperty(
        default=TECH.WGDEF.WIRE,
        doc="waveguide definition for input and output waveguides")
...
class MmiTapered(__MmiTaperPorts__, MmiTaperPorts):
    def define_structure(self):
        twg = WgElDefinition(
            ...
            process=self.end_wg_def.process,
