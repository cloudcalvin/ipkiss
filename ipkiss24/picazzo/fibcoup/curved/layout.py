...
def ConcentricCurvedGrating(min_r_h,
                            min_r_v,
                            h_period,
                            v_period,
                            n_o_trenches,
...
def FiberCouplerConcentricCurvedGrating(min_r_h,
                                        min_r_v,
                                        h_period,
                                        v_period,
                                        n_o_trenches,
                                        ...
                                        socket,
                                        process=TECH.PROCESS.FC,
                                        socket_position=(0.0, 0.0)):
    """ Concentric elliptical curved grating Alle ellipses describe the same angle"""
    grating = ConcentricCurvedGrating(
        min_r_h,
        min_r_v,
        h_period,
        v_period,
        ...
        angle,
        process=TECH.PROCESS.FC)
    return FiberCouplerGrating(
        socket=socket, socket_position=socket_position, grating=grating)


##############################################################


def ConcentricCurvedGratingBox(min_r_h,
                               min_r_v,
                               h_period,
                               v_period,
                               n_o_trenches,
...
def FiberCouplerConcentricCurvedGratingBox(min_r_h,
                                           min_r_v,
                                           h_period,
                                           v_period,
                                           n_o_trenches,
                                           ...
                                           socket_position=(0.0, 0.0)):
    """ Concentric elliptical curved grating fit inside a rectangular box """
    grating = ConcentricCurvedGratingBox(min_r_h, min_r_v, h_period, v_period,
                                         n_o_trenches, trench_width, box_width,
                                         process)
    return FiberCouplerGrating(
        socket=socket, socket_position=socket_position, grating=grating)
...
def ConfocalCurvedGratingBoxGeneric(line_widths_positions,
                                    focus_position,
                                    box_width,
                                    v_h_ratio=1.0,
                                    process=TECH.PROCESS.FC):
    ...
    if v_h_ratio <= 0.0 or v_h_ratio > 1.0:
        raise ValueError(
            "v_h_ratio should be between 0 and 1 in ConfocalCurvedGratingBoxGeneric"
        )
    cd = []
...
def FiberCouplerConfocalCurvedGratingBoxGeneric(line_widths_positions,
                                                focus_position,
                                                box_width,
                                                socket,
                                                v_h_ratio=1.0,
    ...
    """ Confocal elliptical curved grating (all ellipses share the east focus point) fit inside a rectangle
        This can be a set of arbitraty lines and x-positions. The ellipses are made confocal, and the h_v_ratio gives the ratio of the vertical versus the 
        horizontal size of the ellipses. (Should be between zero and one) """
    grating = ConfocalCurvedGratingBoxGeneric(
        line_widths_positions,
        focus_position,
        box_width,
        v_h_ratio,
        process=process)
...
def ConfocalCurvedGratingBox(min_index,
                             h_period,
                             v_period,
                             n_o_trenches,
                             box_width,
...
def FiberCouplerConfocalCurvedGratingBox(min_index,
                                         h_period,
                                         v_period,
                                         n_o_trenches,
                                         box_width,
                                         ...
                                         socket_position=(0.0, 0.0)):
    """ Confocal elliptical curved grating (all ellipses share the east focus point) fit inside a rectangle"""
    grating = ConfocalCurvedGratingBox(
        min_index,
        h_period,
        v_period,
        n_o_trenches,
...
def ConfocalCurvedGratingBoxApodized(min_index,
                                     h_period,
                                     v_period,
                                     n_o_trenches,
                                     box_width,
...
def FiberCouplerConfocalCurvedGratingBoxApodized(min_index,
                                                 h_period,
                                                 v_period,
                                                 n_o_trenches,
                                                 box_width,
...
def MinXConfocalCurvedGratingBox(min_x,
                                 h_period,
                                 v_period,
                                 box_length,
                                 box_width,
    ...
    return ConfocalCurvedGratingBox(min_index, h_period, v_period,
                                    n_o_trenches, box_width, fill_factor,
                                    process)


def FiberCouplerMinXConfocalCurvedGratingBox(min_x,
                                             h_period,
                                             v_period,
                                             box_length,
                                             box_width,
                                             ...
                                             socket_position=(0.0, 0.0)):
    """ Confocal elliptical curved grating (all ellipses share the east focus point) fit inside a rectangle, starting from a given distance from the focus point"""
    grating = MinXConfocalCurvedGratingBox(
        min_x, h_period, v_period, box_length, box_width, fill_factor, process)
    return FiberCouplerGrating(
        socket=socket, socket_position=socket_position, grating=grating)
...
def AngleConfocalCurvedGratingBox(h_period,
                                  v_period,
                                  box_length,
                                  box_width,
                                  angle,
    ...
    return MinXConfocalCurvedGratingBox(min_x, h_period, v_period, box_length,
                                        box_width + extra_width, fill_factor,
                                        process, **kwargs)


def FiberCouplerAutoConfocalCurvedGratingBox(h_period,
                                             v_period,
                                             box_length,
                                             box_width,
                                             socket,
                                             ...
                                             **kwargs):
    """ Confocal elliptical curved grating (all ellipses share the east focus point) fit inside a rectangle, with focus distance calculated from the aperture spread angle"""
    grating = AngleConfocalCurvedGratingBox(
        h_period, v_period, box_length, box_width,
        socket.angle_deg(), fill_factor, extra_width, process)
    return FiberCouplerGrating(
        socket=socket,
...
##############################################################
def MinXConfocalCurvedGratingBoxApodized(min_x,
                                         h_period,
                                         v_period,
                                         box_length,
                                         box_width,
    ...
    max_index = int(ceil((min_x + box_length) / h_period))
    n_o_trenches = max_index - min_index + 1
    return ConfocalCurvedGratingBoxApodized(
        min_index, h_period, v_period, n_o_trenches, box_width, fill_factor,
        apo_func, chirp_func, process)


def AngleConfocalCurvedGratingBoxApodized(h_period,
                                          v_period,
                                          box_length,
                                          box_width,
                                          angle,
    ...
    """ Confocal elliptical curved grating (all ellipses share the east focus point) fit inside a rectangle, with focus distance calculated from the aperture spread angle"""
    min_x = 0.5 * box_width / tan(0.5 * angle * DEG2RAD) - 0.5 * box_length
    return MinXConfocalCurvedGratingBoxApodized(
        min_x, h_period, v_period, box_length, box_width + extra_width,
        fill_factor, apo_func, chirp_func, process, **kwargs)


def FiberCouplerAutoConfocalCurvedGratingBoxApodized(h_period,
                                                     v_period,
                                                     box_length,
                                                     box_width,
                                                     socket,
                                                     ...
                                                     **kwargs):
    """ Confocal elliptical curved grating (all ellipses share the east focus point) fit inside a rectangle, with focus distance calculated from the aperture spread angle"""
    grating = AngleConfocalCurvedGratingBoxApodized(
        h_period, v_period, box_length, box_width,
        socket.angle_deg(), fill_factor, apo_func, chirp_func, extra_width,
        process)
    return FiberCouplerGrating(
        socket=socket,
        socket_position=socket_position,
