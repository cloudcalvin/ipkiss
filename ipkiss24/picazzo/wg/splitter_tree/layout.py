...
class SplitterTreeBasic(Structure):
    ...
    splitter_transforms = DefinitionProperty(
        fdef_name="define_splitter_transforms")
    ...
    def define_connectors_splitter_transforms(self):
        ...
        connectors = [[
            RouteConnectorRounded(
                RouteToEastAtY(
                    input_port=P[0], y_position=-0.5 * self.y_spacing)),
            RouteConnectorRounded(
                RouteToEastAtY(
                    input_port=P[1], y_position=0.5 * self.y_spacing))
        ]]
        ...
        for i in range(self.branch_level):
            ...
            for t in T:
                ...
                c1 = RouteConnectorRounded(
                    RouteToEastAtY(
                        input_port=P_out[0], y_position=P_in[0].y - 0.5 * Y))
                c2 = RouteConnectorRounded(
                    RouteToEastAtY(
                        input_port=P_out[1], y_position=P_in[0].y + 0.5 * Y))
                C += [c1, c2]
