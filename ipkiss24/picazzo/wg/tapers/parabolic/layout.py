...
class WgElTaperParabolic(__WgElTaper__):
    ...
    def __get_taper_shape__(self, start_position, end_position, start_window,
        ...
        avg_offset_start = 0.5 * (
            start_window.start_offset + start_window.end_offset)
        avg_offset_end = 0.5 * (
            end_window.start_offset + end_window.end_offset)

        sp = (start_position[0] + (sinangle * (avg_offset_start)),
              start_position[1] + (cosangle * (avg_offset_start)))
        ep = (end_position[0] + (sinangle * (avg_offset_end)),
              end_position[1] + (cosangle * (avg_offset_end)))

        sp1 = ShapeParabolic(
            begin_coord=sp, end_coord=ep, begin_width=sw, end_width=ew)
        ...
        s1 = Shape([], True)
        if se[0]:
            s1 += [(sp[0] + sinangle * sw / 2.0 - cosangle *
                    (se[0] + TECH.WG.OVERLAP_TRENCH),
                    sp[1] - cosangle * sw / 2.0 - sinangle *
                    (se[0] + TECH.WG.OVERLAP_TRENCH)),
                   (sp[0] - sinangle * sw / 2.0 - cosangle *
                    (se[0] + TECH.WG.OVERLAP_TRENCH),
                    sp[1] + cosangle * sw / 2.0 - sinangle *
                    (se[0] + TECH.WG.OVERLAP_TRENCH))]
        ...
        s1 += sp1a
        if se[1]:
            s1 += [(ep[0] - sinangle * ew / 2.0 + cosangle *
                    (se[1] + TECH.WG.OVERLAP_TRENCH),
                    ep[1] + cosangle * ew / 2.0 + sinangle *
                    (se[1] + TECH.WG.OVERLAP_TRENCH)),
                   (ep[0] + sinangle * ew / 2.0 + cosangle *
                    (se[1] + TECH.WG.OVERLAP_TRENCH),
                    ep[1] - cosangle * ew / 2.0 + sinangle *
                    (se[1] + TECH.WG.OVERLAP_TRENCH))]
