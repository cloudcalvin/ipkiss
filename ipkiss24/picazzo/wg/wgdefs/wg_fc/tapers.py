...
__all__ = [
    "WgElPortTaperFromShallow", "WgElPortTaperToShallow",
    "WgElTaperDeepShallow", "WgElTaperShallowDeep", "WGFCWgElToWgElPortTaper"
]


class WgElTaperDeepShallow(WgElTaperLinear):
    """ Taper from deep to shallow etched waveguides """
    deep_only_length = PositiveNumberProperty(
        allow_none=True, doc="length of taper on deep etch only")
    deep_only_wg_def = DefinitionProperty(fdef_name="define_deep_only_wg_def")
    deep_process = ProcessProperty(
        default=TECH.PROCESS.WG, doc="deep etch process layer")
    shallow_process = ProcessProperty(
        default=TECH.PROCESS.FC, doc="shallow etch process layer")
    deep_only_length_val = DefinitionProperty(
        fdef_name="define_deep_only_length_val")
    ...
    def define_elements(self, elems):
        se = (self.straight_extension[0], max(TECH.TECH.MINIMUM_LINE,
                                              self.straight_extension[1]))
        ## WG
        deep_only_end_position = Coord2(
            self.start_position[0], self.start_position[1]).move_polar(
                self.deep_only_length_val,
                angle_deg(self.end_position, self.start_position))
        taper_wg = WgElTaperLinear(
            start_wg_def=self.start_wg_def,
        ...
        end_window = PathWindow(
            layer=PPLayer(self.shallow_process, TECH.PURPOSE.LF.LINE),
            start_offset=-0.5 * self.end_wg_def.wg_width,
            end_offset=+0.5 * self.end_wg_def.wg_width)
        s1 = self.__get_taper_shape__(self.start_position, self.end_position,
        ...
        start_window = PathWindow(
            ...
            start_offset=-0.5 * self.start_wg_def.wg_width -
            self.start_wg_def.trench_width,
            end_offset=+0.5 * self.start_wg_def.wg_width +
            self.start_wg_def.trench_width)
        end_window = PathWindow(
            layer=PPLayer(self.shallow_process, TECH.PURPOSE.LF_AREA),
            start_offset=-0.5 * self.end_wg_def.wg_width -
            self.end_wg_def.trench_width,
            end_offset=+0.5 * self.end_wg_def.wg_width +
            self.end_wg_def.trench_width)
        s2 = self.__get_taper_shape__(self.start_position, self.end_position,
                                      start_window, end_window,
                                      self.straight_extension)
        elems += Boundary(
            PPLayer(self.shallow_process, TECH.PURPOSE.LF.LINE), s1)
        elems += Boundary(
            PPLayer(self.shallow_process, TECH.PURPOSE.LF_AREA), s2)
        return elems
...
class WgElTaperShallowDeep(WgElTaperDeepShallow):
    ...
    def define_elements(self, elems):
        se = (max(self.straight_extension[0], TECH.TECH.MINIMUM_LINE),
              self.straight_extension[1])

        ## WG
        sp = Coord2(self.end_position[0], self.end_position[1]).move_polar(
            self.deep_only_length_val,
            angle_deg(self.start_position, self.end_position))
        taper_wg = WgElTaperLinear(
            start_wg_def=self.deep_only_wg_def,
    ...
    def define_ports(self, ports):
        ...
        ports += [
            InOpticalPort(
                ...
                angle=(angle + 180.0) % 360.0),
            OutOpticalPort(
                position=self.end_position.move_polar_copy(
                    0.2, angle_deg(self.end_position, self.start_position)),
                wg_definition=self.end_wg_def,
                angle=angle)
        ]
        return ports
...
TECH.WGDEF.AUTO_TAPER_DATA_BASE.add(WGFCWgElDefinition, WgElDefinition,
...
TECH.WGDEF.AUTO_TAPER_DATA_BASE.add(ShallowWgElDefinition, WgElDefinition,
                                    __WgElPortTaperFromShallowAuto__)
