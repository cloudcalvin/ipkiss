...
__all__ = [
    "StructureSimulationVolume2D", "GaussianVolumeSourceAtPort",
    "ModeProfileContinuousSourceAtPort", "get_mode_profile_at_port",
    "FluxplaneAtPort", "ProbingpointAtPort",
    "ModeProfileContinuousSourceAtPosition", "FluxplaneAtPosition",
    "ProbingpointAtPosition", "StructureSimulationDefinition"
]  #DEPRECATED - for backwards compatibility only - to be removed !


class __StructureSimulationVolume__(SimulationVolume1D):
    structure = RestrictedProperty(
        required=True,
        restriction=RestrictType(Structure),
        doc="The ipkiss structure")
    ...
    resolution = IntProperty(
        default=5,
        doc="Resolution for discretisation of the geometry (if relevant)")
    resolution_factor = IntProperty(
        default=1, doc="Additional factor to be applied to the resolution")
    include_growth = FloatProperty(
        default=0.0, doc="absolute additional growth of the simulation volume")
...
class StructureSimulationVolume2D(__StructureSimulationVolume__,
                                  SimulationVolume2D):
    def __init__(self, **kwargs):
        super(StructureSimulationVolume2D, self).__init__(**kwargs)
        LOG.debug("Size of simulation volume : %f x %f" % (self.width,
                                                           self.height))
        ...
        if (not self.has_window_defined()):
            ...
            if ((window_width > width) or (window_width < 0)):
                raise PythonSimulateException(
                    "Invalid value for simulation window (%f): the width should be larger than 0 and smaller than the structure width (=%f)."
                    % (window_width, width))
            return window_width
    ...
    def get_window_height(self):
        ...
        if (not self.has_window_defined()):
            ...
            if ((window_height > height) or (window_height < 0)):
                raise PythonSimulateException(
                    "Invalid vlaue for simulation window (%f): the height should be larger than 0 and smaller than the structure height (=%f)."
    ...
    def get_material_dataset_window(self, resolution=None):
        if not self.has_window_defined():
            ...
            return self.get_material_dataset_for_subset(
                self.get_window_south_west(),
    ...
    def __str__(self):
        name = "%s_2D_R%i_GR%f" % (self.structure.name, self.resolution,
                                   self.include_growth)
        return name


class StructureSimulationVolume3D(__StructureSimulationVolume__,
                                  SimulationVolume3D):
    ...
    def __str__(self):
        name = "%s_3D_R%i_GR%f" % (self.structure.name, self.resolution,
                                   self.include_growth)
        return name
...
class ModeProfileContinuousSourceAtPort(AmplitudeShapedContinuousVolumeSource,
                                        CartesianGeometry2D):
    port = OpticalPortProperty(required=True)
    ...
    save_images = BoolProperty(
        default=True,
        doc=
        "Export images (and csv files) during calculation of the mode profile")
    ...
    material_stack_factory = RestrictedProperty(
        restriction=RestrictType(MaterialStackFactory))
    ...
    def set_structure_simulation_volume(self, structure_simulation_volume):
        ...
        self.resolution = (self.structure_simulation_volume.resolution *
                           self.structure_simulation_volume.resolution_factor)
    ...
    def get_material_array(self):
        if (not hasattr(self, 'structure_simulation_volume')):
            raise PythonSimulateException(
                "Attrbute not set : structure_simulation_volume. Call function 'set_structure_simulation_volume' with a reference to the object of type StructureSimulationVolume2D."
            )
        si = self.size_info
        v = self.structure_simulation_volume
        material_dataset = v.get_material_dataset_for_subset(
            self.south, si.width, si.height, self.port.angle + 180.0)
    ...
    def define_north(self):
        P = self.port.position.move_polar_copy(
            self.port.wg_definition.wg_width / 2.0 +
            self.port.wg_definition.trench_width - (self.grid / 2.0),
            self.port.angle - 90)
        return P

    def define_south(
            self
    ):  #'south' for west-facing port (angle = 180), just a convention
        P = self.port.position.move_polar_copy(
            self.port.wg_definition.wg_width / 2.0 +
            self.port.wg_definition.trench_width - (self.grid / 2.0),
            self.port.angle + 90)
    ...
    def define_mode_profile(self):
        if (not hasattr(self, 'structure_simulation_volume')):
            raise PythonSimulateException(
                "Attribute not set : structure_simulation_volume. Call function 'set_structure_simulation_volume' with a reference to the object of type StructureSimulationVolume2D."
            )
        import camfr
        ...
        engine = CamfrEngine(
            camfr_technology=camfr_technology, save_images=self.save_images)
        engine.set_camfr_settings(camfr_technology)
        (profiles, beta, transmission) = engine.field_for_geometry(
            self,
...
def get_mode_profile_at_port(structure,
                             resolution,
                             port,
                             wavelength,
                             save_images=True):
    ...
    s = ModeProfileContinuousSourceAtPort(
        center_wavelength=wavelength,
        port=port,
        smoothing_width=
        0.0,  #dummy value: we will not actually trigger the source	                                               
...
class ModeProfileContinuousSourceAtPosition(ModeProfileContinuousSourceAtPort):
    ...
    def define_port(self):
        return OpticalPort(
            ...
            angle=self.angle,
            wg_definition=WgElDefinition(
                wg_width=self.width / 2.0, trench_width=0.0))
...
class FluxplaneAtPort(Fluxplane):
    ...
    def define_north(self):
        if (self.overlap_trench):
            ...
            tw = 0
        P = self.port.position.move_polar_copy(
            self.port.wg_definition.wg_width / 2.0 + tw, self.port.angle - 90)
        return P
...
class StructureSimulationDefinition(SimulationDefinition):
    ...
    def __get_simulation_dimension__(self, simul_params):
        if ("dimensions" in simul_params):
            ...
            if (dim in [2, 3]):
                ...
                raise Exception(
                    "Invalid value for simulation parameter 'dimensions': should be 2 or 3."
                )
        else:
            return 2
    ...
    def define_landscape(self):
        ...
        if (dim == 2):
            simulation_volume = StructureSimulationVolume2D(
            ...
            simulation_volume = StructureSimulationVolume3D(
                simul_params=self.simul_params)
        ...
        if "step_processor" in self.simul_params:
            ...
            if (isinstance(p, SaveFieldsHDF5Processor)):
                p.fileName = ls.simulation_id + "_" + str(
                    p.field_component) + ".h5"
