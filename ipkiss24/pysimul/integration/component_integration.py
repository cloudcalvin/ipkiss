...
class __StructureSimulationVolume__(SimulationVolume1D):
    structure = RestrictedProperty(
        required=True, restriction=RestrictType(Structure), doc="The ipkiss structure"
    )
    ...
    resolution = IntProperty(
        default=5, doc="Resolution for discretisation of the geometry (if relevant)"
    )
...
class ModeProfileContinuousSourceAtPort(
    AmplitudeShapedContinuousVolumeSource, CartesianGeometry2D
):
    port = OpticalPortProperty(required=True)
    ...
    save_images = BoolProperty(
        default=True,
        doc="Export images (and csv files) during calculation of the mode profile",
    )
    ...
    def set_structure_simulation_volume(self, structure_simulation_volume):
        ...
        self.resolution = (
            self.structure_simulation_volume.resolution
    ...
    def define_north(self):
        P = self.port.position.move_polar_copy(
            ...
            + self.port.wg_definition.trench_width
            - (self.grid / 2.0),
            self.port.angle - 90,
        )
        return P
    ...
    def define_mode_profile(self):
        ...
        if self.polarization == TE:
            camfr_technology.POLARIZATION = (
                camfr.TM
            )  # use for top-down simulations : #TE-mode in 3D -> TM-mode for 2D upper surface view -> extract Hz component
        elif self.polarization == TM:
            camfr_technology.POLARIZATION = (
                camfr.TE
            )  # use for top-down simulations : #TM-mode in 3D -> TE-mode for 2D upper surface view -> extract Ez component
        else:
...
class StructureSimulationDefinition(SimulationDefinition):
    ...
    def define_landscape(self):
        ...
        ls = SimulationLandscape(
            simulation_volume=simulation_volume, simul_params=self.simul_params
        )
...
PicazzoStructureSimulationDefinition = (
    StructureSimulationDefinition
)  # DEPRECATED - for backwards compatibility only - to be removed
