    ...
    def initialise_engine(self, pLandscape):
        ...
        if not isinstance(pLandscape, SimulationLandscape):
            raise InvalidArgumentException(
                "Invalid argument for function setLandscape:: not of type runtime.basic.Landscape."
            )

    def get_procedure_class(self):
        raise NotImplementedException(
            "get_procedure_class not implemented in class __SimulationEngine__"
        )


class FDTDEngine(__SimulationEngine__):
    resolution = RestrictedProperty(
        required=True,
        restriction=RESTRICT_INT & RESTRICT_POSITIVE,
        doc="Resolution of the FDTD simulation")

    def exportDielectricH5(self, fileName=None):
        '''Export the EPS values of the dielectric to a HDF5 output file'''
        raise NotImplementedException(
            "exportDielectricH5 not implemented in class FDTDEngine")

    def get_material_dataset(self):
        '''Get a dataset with the EPS values of the dielectric, as it was processed by the engine '''
        raise NotImplementedException(
            "get_material_dataset not implemented in class FDTDEngine")
    ...
    def getFieldAmplitudeAtMonitorPoint(self, pCoord, pComp):
        ...
        raise NotImplementedException(
            "getFieldAmplitudeAtMonitorPoint not implemented in class FDTDEngine"
        )

    def step(self):
        '''Perform a step in the simulation'''
        raise NotImplementedException(
            "step not implemented in class FDTDEngine")

    def prepareHDF5File(self, filename):
        raise NotImplementedException(
            "prepareHDF5File not implemented in class FDTDEngine")

    def writeFieldsToHDF5File(self, pFileRef, pComp):
        raise NotImplementedException(
            "writeFieldsToHDF5File not implemented in class FDTDEngine")

    def closeHDF5File(self, pFileRef):
        raise NotImplementedException(
            "closeHDF5File not implemented in class FDTDEngine")
...
class ModeSolverEngine(__SimulationEngine__):
    def mode_profile_of_port(self, port):
        raise NotImplementedException(
            "mode_profile_of_port not implemented in abstract superclass ModeSolverEngine"
        )
