...
class MeepScripter(FDTDEngine):
    use_averaging = BoolProperty(
        default=True, doc="Use subpixel averaging in Meep.")
    is_nonlinear = BoolProperty(
        default=False, doc="When True, use nonlinear chi3. Default False.")
    ...
    symmY = BoolProperty(
        default=False,
        doc=
        "Use symmetry in Y-direction? Default = False (no symmetry). See python-meep doc."
    )
...
try:
	...
	""")
        ...
        self.dim = self.__createMeepComputationalVolume__(
            landscape.simulation_volume)
        self.__script__("#callback for epsilon-values")
        if self.dim == 2:
            materials_2d_filename = name_template + ".materials2D.pysimul"
            self.__script__(
                "from pysimul.runtime.MeepFDTD.meep_scripter_runtime import MeepMaterial2DPolygonsFromFile"
            )
            self.__script__(
                "material = MeepMaterial2DPolygonsFromFile(config_file = \"%s\", meep_volume = meep_vol)"
                % (materials_2d_filename))
            ...
            self.__script__(
                "from pysimul.runtime.MeepFDTD.meep_scripter_runtime import MeepMaterial3DPolygonsFromFile"
            )
            self.__script__(
                "material = MeepMaterial3DPolygonsFromFile(config_file = \"%s\", meep_volume = meep_vol)"
                % (materials_3d_filename))
            self.__script__("meep.set_EPS_Callback(material.__disown__())")
            material_stacks_shapely_polygons = landscape.simulation_volume.geometry.material_stacks_shapely_polygons
            bitmap_polygons = []
            material_stack_ids = []
        ...
        if landscape.pml_thickness > 0.0:
            ...
            if isinstance(landscape.pml_direction, str):
                ...
                self.__script__(
                    "pml = meep.pml(%f, direction)" % landscape.pml_thickness)
            else:
        ...
        self.__script__(
            "structure = meep.structure(meep_vol , meep.EPS, pml, symmetry=symmetry_object )"
        )
    ...
    def __createMeepComputationalVolume__(self, volume):
        ...
        if isinstance(volume, SimulationVolume3D):
            self.__script__("meep_vol = meep.vol3d(%f,%f,%f,%i)" %
                            (volume.window_width, volume.window_height,
                             volume.size_z, self.resolution))
            return 3
        if isinstance(volume, SimulationVolume2D):
            self.__script__("meep_vol = meep.vol2d(%f,%f,%i)" %
                            (volume.window_width, volume.window_height,
                             self.resolution))
            return 2
        if isinstance(volume, SimulationVolume1D):
            self.__script__("meep_vol = meep.vol1d(%f,%i)" %
                            (volume.window_width, self.resolution))
            return 1
    ...
    def __calc_meep_coord__(self, coord):
        ...
        accuracy = int(
            math.ceil(math.log10(1.0 / (1.0 / self.resolution / 100))))
        c = coord - sw
        x = round(
            c.x, accuracy
        )  #because of floating point accuracy erros with Python, whereby sometimes a wrong value is then sent to Meep
        if (self.dim == 3):
    ...
    def __make_meep_vec__(self, coord):
        ...
        if (self.dim == 3):
            ...
            raise PythonSimulateException(
                "Invalid value for self.dim : expected 1, 2 or 3. The value is : %s"
                % str(self.dim))
    ...
    def __makeMeepComponent__(self, comp):
        if comp == compEx:
            ...
            return "meep.Hz"
        raise PythonSimulateException(
            "Unknown component. Must be Ex, Ey, Ez, Hx, Hy or Hz.")
    ...
    def __addMeepSource__(self, src, landscape, seq_nr, name_template):
        ...
        meepSource = None
        self.__script__("src%i_center_wavelength = %f" %
                        (seq_nr, src.center_wavelength))
        self.__script__(
            "src%i_center_freq = 1.0 / (float(src%i_center_wavelength) / 1000.0)"
            % (seq_nr, seq_nr))
        if isinstance(src, __GaussianSource__):
            self.__script__("src%i_pulse_width_wl = %f" % (seq_nr,
                                                           src.pulse_width))
            self.__script__(
                "src%i_pulse_width_freq = ( (float(src%i_pulse_width_wl)/1000.0) / (float(src%i_center_wavelength)/1000.0) ) * src%i_center_freq"
                % (seq_nr, seq_nr, seq_nr, seq_nr))
            self.__script__(
                "meepSource%i = meep.gaussian_src_time(src%i_center_freq, src%i_pulse_width_freq)"
                % (seq_nr, seq_nr, seq_nr))
        if isinstance(src, __ContinuousSource__):
            self.__script__(
                "meepSource%i = meep.continuous_src_time(src%i_center_freq, %f, %f, %f, %f)"
                % (seq_nr, seq_nr, src.smoothing_width, src.start_time,
        ...
        if isinstance(src, __EMPointSource__):
            ...
            if isinstance(src, __AmplitudeShapedSource__):
                ...
                if self.ampl_shaping_counter > 1:
                    raise Exception(
                        "Amplitude shaping is supported for maiximum 1 source..."
                    )
                file_name = name_template + ".ampl_src%i.pysimul" % seq_nr
                self.__script__(
                    "from pysimul.runtime.MeepFDTD.meep_scripter_runtime import AmplitudeFactorFromFile"
                )
                self.__script__(
                    "ampl = AmplitudeFactorFromFile(config_file = \"%s\")" %
                    file_name)
                ...
                self.__script__(
                    "meep_fields.add_volume_source(%s, meepSource%i, meepSrcVol%i, %f)"
                    % (meepComp, seq_nr, seq_nr, src.amplitude))
        else:
            raise NotImplementedException(
                "Unexpected case in MeepSimulationEngine::__addMeepSource__")
    ...
    def __addMeepFluxplane__(self, flx, seq_nr=1):
        ...
        self.__script__("flx%i_vol = meep.volume(%s,%s)" % (seq_nr, vec1,
                                                            vec2))
        self.__script__("flx%i_center_wavelength = %f" %
                        (seq_nr, flx.center_wavelength))
        self.__script__(
            "flx%i_center_freq = 1.0 / (float(flx%i_center_wavelength) / 1000.0)"
            % (seq_nr, seq_nr))
        self.__script__("flx%i_pulse_width_wl = %f" % (seq_nr,
                                                       flx.pulse_width))
        self.__script__(
            "flx%i_pulse_width_freq = ( (float(flx%i_pulse_width_wl)/1000.0) / (float(flx%i_center_wavelength)/1000.0) ) * flx%i_center_freq"
            % (seq_nr, seq_nr, seq_nr, seq_nr))
        self.__script__(
            "flx%i_max_freq = flx%i_center_freq + flx%i_pulse_width_freq / 2.0"
            % (seq_nr, seq_nr, seq_nr))
        self.__script__(
            "flx%i_min_freq =  flx%i_center_freq - flx%i_pulse_width_freq / 2.0"
            % (seq_nr, seq_nr, seq_nr))
        self.__script__(
            "flx%i_plane = meep_fields.add_dft_flux_plane(flx%i_vol, flx%i_min_freq, flx%i_max_freq, %i )"
            % (seq_nr, seq_nr, seq_nr, seq_nr, flx.number_of_sampling_freq))
        if not (flx.init_hdf5 is None):
            self.__script__("flx%i_plane.load_hdf5(meep_fields, \"%s\")" %
                            (seq_nr, flx.init_hdf5.replace(".h5", "")))

    def __collectAndSaveFluxplane__(self, filename_prefix, flx, seq_nr=1):
        self.__script__(
            "#collect flux data and save it to file, so it can be retrieved later for analysis"
        )
        self.__script__("flux%i_data = meep.getFluxData(flx%i_plane)" %
                        (seq_nr, seq_nr))
        self.__script__(
            "flux%i_file_handle = open(\"%s\",'w')" %
            (seq_nr,
             filename_prefix + ".fluxplane_" + flx.name.replace(" ", "_")))
        self.__script__("dump(flux%i_data, flux%i_file_handle)" % (seq_nr,
                                                                   seq_nr))
    ...
    def __addMeepProbingPoint__(self, c, seq_nr):
        ...
        self.__script__("pp%i_coord = %s" % (seq_nr,
                                             self.__make_meep_vec__(c.point)))
    ...
    def __exportDielectricH5__(self, filename=None):
        ...
        self.__script__(
            "\tdielectric_file_handle = meep.prepareHDF5File(\"%s\")" %
            (filename))
        self.__script__(
            "\tmeep_fields.output_hdf5(meep.Dielectric, meep_vol.surroundings(), dielectric_file_handle)"
        )
...
class MeepScripterProcedure(FDTDFieldCalculationProcedure):
    ...
    def run(self, interactive_mode=False, append_script="",
            name_template=None):
        ...
        self.engine.init_script(
            self.landscape, self.__script__, name_template=name_template)
        if isinstance(self.stopcriterium, RunUntilFieldsDecayedAtProbingPoint):
            self.__script__("#stop criterium")
            self.__script__("stopcriterium_field_component = %s" %
                            self.engine.__makeMeepComponent__(
                                self.stopcriterium.field_component))
            self.__script__("stopcriterium_probing_point = %s" %
                            self.engine.__make_meep_vec__(
                                self.stopcriterium.probingpoint.point))
            self.__script__(
                "h5_field_file_handle = meep.prepareHDF5File(\"%s\")" %
                (name_template + "." +
                 str(self.stopcriterium.field_component) + ".h5"))
            stopcriterium_txt = "meep.runUntilFieldsDecayed(meep_fields, meep_vol, stopcriterium_field_component, stopcriterium_probing_point, pDecayFactor = %f, pHDF5OutputFile = h5_field_file_handle" % (
                self.stopcriterium.decay_factor)
            if isinstance(self.step_processor, SaveFieldsHDF5Processor):
                if self.step_processor.field_component != self.stopcriterium.field_component:
                    raise Exception(
                        "MeepScripterProcedure can only handle a combination of RunUntilFieldsDecayedAtProbingPoint and SaveFieldsHDF5Processor with both the same field component parameter."
                    )
                self.__script__(stopcriterium_txt +
                                ", pH5OutputIntervalSteps = %i)" %
                                (self.step_processor.H5OutputIntervalSteps))
            else:
                self.__script__(stopcriterium_txt + ")")
        elif isinstance(self.stopcriterium, StopAfterSteps):
            self.__script__(
                "#stop after %i steps" % self.stopcriterium.maximumSteps)
            if isinstance(self.step_processor, SaveFieldsHDF5Processor):
                self.__script__(
                    "h5_field_file_handle = meep.prepareHDF5File(\"%s\")" %
                    (name_template + "." +
                     str(self.step_processor.field_component) + ".h5"))
            ...
            self.__script__(
                "	print \"Simulation is at step : %i \"%step_count")
            if isinstance(self.step_processor, SaveFieldsHDF5Processor):
                self.__script__("    if step_count %" + " %i == 0:" %
                                self.step_processor.H5OutputIntervalSteps)
                self.__script__(
                    "		meep_fields.output_hdf5(%s, meep_vol.surroundings(), h5_field_file_handle, 1)"
                    % self.engine.__makeMeepComponent__(
                        self.step_processor.field_component))
            self.__script__("    step_count = step_count + 1")
        else:
            if not isinstance(self.stopcriterium, __StopCriterium__):
                self.__script__(
                    "#STOPCRITERIUM : type %s was not yet implemented : manually add the code for your stopcriterium"
                    % type(self.stopcriterium))
