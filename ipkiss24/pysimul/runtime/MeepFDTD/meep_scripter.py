...
from pysimul.runtime.basic import (
    __SimulationVolume__,
    __EMSource__,
    __GaussianSource__,
    __ContinuousSource__,
...
class MeepScripter(FDTDEngine):
    ...
    symmY = BoolProperty(
        default=False,
        doc="Use symmetry in Y-direction? Default = False (no symmetry). See python-meep doc.",
    )
...
try:
	...
	"""
        ...
        if self.dim == 2:
            ...
            material_stacks_shapely_polygons = (
                landscape.simulation_volume.geometry.material_stacks_shapely_polygons
            )
            ...
            for material_stack_id, bitmap_polygon in material_stacks_shapely_polygons:
                ...
                eps_values.append(
                    landscape.simulation_volume.geometry.material_stack_factory[
                        material_stack_id
            ...
            south_west = landscape.simulation_volume.size_info.south_west
            material_stacks_npy = (
                landscape.simulation_volume.geometry.get_numpy_matrix_representation_of_all_material_stacks()
            )
            n_o_material_stacks = (
                landscape.simulation_volume.geometry.get_number_of_material_stacks_in_store()
            )
            from pickle import dump
        ...
        if landscape.pml_thickness > 0.0:
            ...
            if isinstance(landscape.pml_direction, str):
                ...
                if dirint == 0:
                    self.__script__("direction = meep.X")
                if dirint == 1:
                    self.__script__("direction = meep.Y")
                if dirint == 2:
                    self.__script__("direction = meep.Z")
                self.__script__(
    ...
    def __createMeepComputationalVolume__(self, volume):
        ...
        if isinstance(volume, SimulationVolume3D):
            self.__script__(
                "meep_vol = meep.vol3d(%f,%f,%f,%i)"
                % (
                    volume.window_width,
                    volume.window_height,
                    volume.size_z,
                    self.resolution,
                )
            )
        ...
        if isinstance(volume, SimulationVolume2D):
            self.__script__(
                "meep_vol = meep.vol2d(%f,%f,%i)"
                % (volume.window_width, volume.window_height, self.resolution)
            )
            return 2
        if isinstance(volume, SimulationVolume1D):
            self.__script__(
                "meep_vol = meep.vol1d(%f,%i)" % (volume.window_width, self.resolution)
            )
            return 1
    ...
    def __addMeepSource__(self, src, landscape, seq_nr, name_template):
        ...
        meepSource = None
        self.__script__(
            "src%i_center_wavelength = %f" % (seq_nr, src.center_wavelength)
        )
        self.__script__(
        ...
        if isinstance(src, __EMPointSource__):
            ...
            if isinstance(src, __AmplitudeShapedSource__):
                ...
                self.__script__(
                    'ampl = AmplitudeFactorFromFile(config_file = "%s")' % file_name
                )
    ...
    def __addMeepFluxplane__(self, flx, seq_nr=1):
        ...
        self.__script__(
            "flx%i_center_wavelength = %f" % (seq_nr, flx.center_wavelength)
        )
        self.__script__(
        ...
        if not (flx.init_hdf5 is None):
            self.__script__(
                'flx%i_plane.load_hdf5(meep_fields, "%s")'
    ...
    def __collectAndSaveFluxplane__(self, filename_prefix, flx, seq_nr=1):
        ...
        self.__script__(
            "flux%i_data = meep.getFluxData(flx%i_plane)" % (seq_nr, seq_nr)
        )
        self.__script__(
            "flux%i_file_handle = open(\"%s\",'w')"
            % (seq_nr, filename_prefix + ".fluxplane_" + flx.name.replace(" ", "_"))
        )
    ...
    def __exportDielectricH5__(self, filename=None):
        ...
        self.__script__(
            '\tdielectric_file_handle = meep.prepareHDF5File("%s")' % (filename)
        )
        self.__script__(
...
class MeepScripterProcedure(FDTDFieldCalculationProcedure):
    ...
    def run(self, interactive_mode=False, append_script="", name_template=None):
        ...
        if isinstance(self.stopcriterium, RunUntilFieldsDecayedAtProbingPoint):
            ...
            self.__script__(
                "stopcriterium_field_component = %s"
                % self.engine.__makeMeepComponent__(self.stopcriterium.field_component)
            )
            self.__script__(
                "stopcriterium_probing_point = %s"
                % self.engine.__make_meep_vec__(self.stopcriterium.probingpoint.point)
            )
            self.__script__(
                ...
                % (
                    name_template
                    + "."
                    + str(self.stopcriterium.field_component)
                    + ".h5"
                )
            )
            stopcriterium_txt = (
                "meep.runUntilFieldsDecayed(meep_fields, meep_vol, stopcriterium_field_component, stopcriterium_probing_point, pDecayFactor = %f, pHDF5OutputFile = h5_field_file_handle"
            ...
            if isinstance(self.step_processor, SaveFieldsHDF5Processor):
                self.__script__(
                    ...
                    % (
                        name_template
                        + "."
                        + str(self.step_processor.field_component)
                        + ".h5"
                    )
                )
            ...
            if isinstance(self.step_processor, SaveFieldsHDF5Processor):
                self.__script__(
                    "    if step_count %"
                    + " %i == 0:" % self.step_processor.H5OutputIntervalSteps
                )
                self.__script__(
