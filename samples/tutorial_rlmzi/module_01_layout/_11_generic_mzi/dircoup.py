...
class DirectionalCoupler(Structure):
    ...
    wg_def_1 = WaveguideDefProperty(
        default=TECH.WGDEF.WIRE, doc="waveguide definition for upper arm"
    )
    wg_def_2 = WaveguideDefProperty(
        default=TECH.WGDEF.WIRE, doc="waveguide definition for lower arm"
    )
    coupler_spacing = PositiveNumberProperty(
        default=TECH.WG.DC_SPACING, doc="spacing between the two waveguide centerlines"
    )
    coupler_length = PositiveNumberProperty(
        default=TECH.WG.SHORT_STRAIGHT, doc="length of the directional coupler"
    )

    def validate_properties(self):
        """ check whether the combination of properties is valid """
        if self.coupler_spacing <= 0.5 * (
            self.wg_def_1.wg_width + self.wg_def_2.wg_width
        ):
    ...
    def get_waveguide_routes(self):
        ...
        r1 = Route(
            points=[
                (0, 0.5 * self.coupler_spacing),
                (self.coupler_length, 0.5 * self.coupler_spacing),
            ],
            input_port=InOpticalPort(
                position=(0, 0.5 * self.coupler_spacing),
                angle=180.0,
...
class BentDirectionalCoupler(DirectionalCoupler):
    ...
    bend_angle = AngleProperty(
        default=45.0, restriction=RestrictRange(0, 90, True, True)
    )  # angle between 0 and 90 degrees, including 0 and 90
    ...
    def get_waveguide_routes(self):
        ...
        r1_start = RouteToAngle(
            input_port=r1s.ports[0],
            angle_out=180.0 - self.bend_angle,
            bend_radius=self.bend_radius,
            start_straight=0.0,
            end_straight=self.straight_after_bend,
        )
        r1_end = RouteToAngle(
            input_port=r1s.ports[-1],
            angle_out=self.bend_angle,
            bend_radius=self.bend_radius,
            start_straight=0.0,
            end_straight=self.straight_after_bend,
        )
        ...
        r2_start = RouteToAngle(
            input_port=r2s.ports[0],
            angle_out=180.0 + self.bend_angle,
            bend_radius=self.bend_radius,
            start_straight=0.0,
        )
        r2_end = RouteToAngle(
            input_port=r2s.ports[-1],
            angle_out=-self.bend_angle,
            bend_radius=self.bend_radius,
            start_straight=0.0,
        )
