...
class Ring(Structure):
    radius = FloatProperty(
        ...
        restriction=RestrictRange(
            lower=10.0, upper=15.0, lower_inc=True, upper_inc=True
        ),
    )
...
class ArrayOfRings(Structure):
    ...
    n_of_rings_height = PositiveNumberProperty(
        required=True,
        restriction=RestrictRange(lower=5, upper=7, lower_inc=True, upper_inc=True),
    )
    ...
    def define_elements(self, elems):
        ...
        elems += ARef(
            ...
            origin=self.origin,
            period=(delta, delta),
            n_o_periods=(self.n_of_rings_width, self.n_of_rings_height),
        )
        return elems
...
s = ArrayOfRings(
    ring_radius=11.563, n_of_rings_width=41, n_of_rings_height=6, origin=(0.0, 0.0)
)
