...
class MmiSweep(Structure):
    def get_mmi(self, width, length, wg_offset, taper_width):
        ...
        return Mmi2x2Tapered(
            width=width, length=length, wg_offset=wg_offset, taper_width=taper_width
        )
...
class RingSweep(Structure):
    def get_ring(self, radius):
        ...
        return RingRect180DropFilter(
            bend_radius=radius, straights=(0.0, 0.0), coupler_spacings=[0.67, 0.67]
        )
